using System;
using AssetRipper.Conversions.FastPng.GlobalFunctions;
using AssetRipper.Conversions.FastPng.GlobalVariables;
using AssetRipper.Conversions.FastPng.Helpers;
using AssetRipper.Conversions.FastPng.InlineArrays;
using AssetRipper.Conversions.FastPng.Intrinsics.Implemented;
using AssetRipper.Conversions.FastPng.Intrinsics.Unimplemented;
using AssetRipper.Conversions.FastPng.Structures;

namespace AssetRipper.Conversions.FastPng;

public static partial class FPng
{
	[MangledName("_Avx2WmemEnabledWeakValue")]
	[DemangledName("_Avx2WmemEnabledWeakValue")]
	public static int Avx2WmemEnabledWeakValue
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.Avx2WmemEnabledWeakValue.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.Avx2WmemEnabledWeakValue.Value = value;
		}
	}

	[MangledName("??_C@_1DK@OKEJEHAG@?$AA?4?$AA?4?$AA?1?$AA?4?$AA?4?$AA?1?$AA?4?$AA?4?$AA?1?$AA?4?$AA?4?$AA?1?$AAS?$AAa?$AAm?$AAp?$AAl?$AAe?$AAs?$AA?2?$AAf?$AAp?$AAn?$AAg?$AA?4?$AAc?$AAp?$AAp?$AA?$AA@")]
	[DemangledName("L\"../../../../Samples\\\\fpng.cpp\"")]
	[CleanName("String")]
	public static InlineArray_29_oeds4ea String_uxbkrtb
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_uxbkrtb.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_uxbkrtb.Value = value;
		}
	}

	[MangledName("??_C@_13COJANIEC@?$AA0?$AA?$AA@")]
	[DemangledName("L\"0\"")]
	[CleanName("String")]
	public static InlineArray_2_oeds4ea String_kdrammb
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_kdrammb.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_kdrammb.Value = value;
		}
	}

	[MangledName("??_C@_1EA@NGEOLJKL@?$AAt?$AAe?$AAm?$AAp?$AA_?$AAb?$AAu?$AAf?$AA_?$AAo?$AAf?$AAs?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AAt?$AAe?$AAm?$AAp?$AA_?$AAb?$AAu?$AAf?$AA?4?$AAs?$AAi?$AAz?$AAe?$AA?$CI?$AA?$CJ?$AA?$AA@")]
	[DemangledName("L\"temp_buf_ofs <= temp_buf.size()\"")]
	[CleanName("String")]
	public static InlineArray_32_oeds4ea String_bik4erc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_bik4erc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_bik4erc.Value = value;
		}
	}

	[MangledName("??_C@_1FA@FOPIPCKM@?$AA?$CI?$AAo?$AAu?$AAt?$AA_?$AAo?$AAf?$AAs?$AA?5?$AA?$CL?$AA?5?$AAz?$AAl?$AAi?$AAb?$AA_?$AAs?$AAi?$AAz?$AAe?$AA?$CJ?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AAo?$AAu?$AAt?$AA_?$AAb?$AAu?$AAf@")]
	[DemangledName("L\"(out_ofs + zlib_size) <= out_buf\"...")]
	[CleanName("String")]
	public static InlineArray_40_oeds4ea String_dqvhqxc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_dqvhqxc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_dqvhqxc.Value = value;
		}
	}

	[MangledName("?s_color_type@?1??fpng_encode_image_to_memory@fpng@@YA_NPEBXIIIAEAV?$vector@EV?$allocator@E@std@@@std@@I@Z@4QBEB")]
	[DemangledName("unsigned char const *const `bool __cdecl fpng::fpng_encode_image_to_memory(void const *, unsigned int, unsigned int, unsigned int, class std::vector<unsigned char, class std::allocator<unsigned char>> &, unsigned int)'::`2'::s_color_type")]
	public static InlineArray_5_w3otm9c s_color_type
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.s_color_type.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.s_color_type.Value = value;
		}
	}

	[MangledName("??_C@_0BB@BKMJLBGB@?$AA?$AA?$AA?$AA?$AA?$AA?$AA?$AAIEND?$KOB?$GA?$IC?$AA@")]
	[DemangledName("\"\\0\\0\\0\\0\\0\\0\\0\\0IEND\\xAEB`\\x82\"")]
	[CleanName("String")]
	public static InlineArray_17_w3otm9c String_nz3h57a
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_nz3h57a.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_nz3h57a.Value = value;
		}
	}

	[MangledName("?g_crc32_4@fpng@@3QAY0BAA@$$CBIA")]
	[DemangledName("unsigned int const (*const fpng::g_crc32_4)[256]")]
	public static InlineArray_4_vjlif9a g_crc32_4
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_crc32_4.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_crc32_4.Value = value;
		}
	}

	[MangledName("??_C@_1BE@DFLAPPIJ@?$AAp?$AAP?$AAr?$AAe?$AAv?$AA_?$AAs?$AAr?$AAc?$AA?$AA@")]
	[DemangledName("L\"pPrev_src\"")]
	[CleanName("String")]
	public static InlineArray_10_oeds4ea String_uc52kza
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_uc52kza.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_uc52kza.Value = value;
		}
	}

	[MangledName("??_C@_1CM@PHNBINKF@?$AA?$CI?$AAl?$AA?$CJ?$AA?5?$AA?$DO?$AA?$DN?$AA?5?$AA0?$AA?5?$AA?$CG?$AA?$CG?$AA?5?$AA?$CI?$AAl?$AA?$CJ?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA1?$AA6?$AA?$AA@")]
	[DemangledName("L\"(l) >= 0 && (l) <= 16\"")]
	[CleanName("String")]
	public static InlineArray_22_oeds4ea String_9hewgtd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_9hewgtd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_9hewgtd.Value = value;
		}
	}

	[MangledName("??_C@_1CI@EDFEPIBM@?$AA?$CI?$AAb?$AA?$CJ?$AA?5?$AA?$DM?$AA?5?$AA?$CI?$AA1?$AAU?$AAL?$AAL?$AA?5?$AA?$DM?$AA?$DM?$AA?5?$AA?$CI?$AAl?$AA?$CJ?$AA?$CJ?$AA?$AA@")]
	[DemangledName("L\"(b) < (1ULL << (l))\"")]
	[CleanName("String")]
	public static InlineArray_20_oeds4ea String_ooiduua
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_ooiduua.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_ooiduua.Value = value;
		}
	}

	[MangledName("??_C@_1CG@LKELDOLB@?$AAb?$AAi?$AAt?$AA_?$AAb?$AAu?$AAf?$AA_?$AAs?$AAi?$AAz?$AAe?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA6?$AA4?$AA?$AA@")]
	[DemangledName("L\"bit_buf_size <= 64\"")]
	[CleanName("String")]
	public static InlineArray_19_oeds4ea String_mggwlgb
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_mggwlgb.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_mggwlgb.Value = value;
		}
	}

	[MangledName("?g_defl_len_sym@fpng@@3QBGB")]
	[DemangledName("unsigned short const *const fpng::g_defl_len_sym")]
	public static InlineArray_256_oeds4ea g_defl_len_sym
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_defl_len_sym.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_defl_len_sym.Value = value;
		}
	}

	[MangledName("??_C@_1CG@PKEJNHCM@?$AAs?$AAr?$AAc?$AA_?$AAo?$AAf?$AAs?$AA?5?$AA?$DN?$AA?$DN?$AA?5?$AAh?$AA?5?$AA?$CK?$AA?5?$AAb?$AAp?$AAl?$AA?$AA@")]
	[DemangledName("L\"src_ofs == h * bpl\"")]
	[CleanName("String")]
	public static InlineArray_19_oeds4ea String_bjvsuxc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_bjvsuxc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_bjvsuxc.Value = value;
		}
	}

	[MangledName("??_C@_1DI@FDHIJIML@?$AAt?$AAo?$AAt?$AAa?$AAl?$AA_?$AAc?$AAo?$AAd?$AAe?$AAs?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AAc?$AAo?$AAd?$AAe?$AAs?$AA?4?$AAs?$AAi?$AAz?$AAe?$AA?$CI?$AA?$CJ?$AA?$AA@")]
	[DemangledName("L\"total_codes <= codes.size()\"")]
	[CleanName("String")]
	public static InlineArray_28_oeds4ea String_hsgehfc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_hsgehfc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_hsgehfc.Value = value;
		}
	}

	[MangledName("??_C@_1CE@GPOANAFC@?$AAb?$AAi?$AAt?$AA_?$AAb?$AAu?$AAf?$AA_?$AAs?$AAi?$AAz?$AAe?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA7?$AA?$AA@")]
	[DemangledName("L\"bit_buf_size <= 7\"")]
	[CleanName("String")]
	public static InlineArray_18_oeds4ea String_vpte4cc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_vpte4cc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_vpte4cc.Value = value;
		}
	}

	[MangledName("??_C@_1JI@IFFHKEFF@?$AAd?$AAh?$AA?4?$AAm?$AA_?$AAh?$AAu?$AAf?$AAf?$AA_?$AAc?$AAo?$AAd?$AAe?$AAs?$AA?$FL?$AA1?$AA?$FN?$AA?$FL?$AAd?$AAi?$AAs?$AAt?$AA_?$AAs?$AAy?$AAm?$AA?$FN?$AA?5?$AA?$DN?$AA?$DN?$AA?5@")]
	[DemangledName("L\"dh.m_huff_codes[1][dist_sym] == \"...")]
	[CleanName("String")]
	public static InlineArray_76_oeds4ea String_df5ko9b
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_df5ko9b.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_df5ko9b.Value = value;
		}
	}

	[MangledName("??_C@_1CM@MLLBGOKN@?$AA?$CI?$AAl?$AA?$CJ?$AA?5?$AA?$DO?$AA?$DN?$AA?5?$AA1?$AA?5?$AA?$CG?$AA?$CG?$AA?5?$AA?$CI?$AAl?$AA?$CJ?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA1?$AA6?$AA?$AA@")]
	[DemangledName("L\"(l) >= 1 && (l) <= 16\"")]
	[CleanName("String")]
	public static InlineArray_22_oeds4ea String_isj2lib
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_isj2lib.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_isj2lib.Value = value;
		}
	}

	[MangledName("?g_bitmasks@fpng@@3QBIB")]
	[DemangledName("unsigned int const *const fpng::g_bitmasks")]
	public static InlineArray_17_vqkfj7a g_bitmasks
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_bitmasks.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_bitmasks.Value = value;
		}
	}

	[MangledName("?g_defl_len_extra@fpng@@3QBEB")]
	[DemangledName("unsigned char const *const fpng::g_defl_len_extra")]
	public static InlineArray_256_w3otm9c g_defl_len_extra
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_defl_len_extra.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_defl_len_extra.Value = value;
		}
	}

	[MangledName("??_C@_1CG@JFLKAPAP@?$AAb?$AAi?$AAt?$AAs?$AA_?$AAt?$AAo?$AA_?$AAs?$AAh?$AAi?$AAf?$AAt?$AA?5?$AA?$DM?$AA?5?$AA6?$AA4?$AA?$AA@")]
	[DemangledName("L\"bits_to_shift < 64\"")]
	[CleanName("String")]
	public static InlineArray_19_oeds4ea String_2ej5tjb
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_2ej5tjb.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_2ej5tjb.Value = value;
		}
	}

	[MangledName("??_C@_0BA@FOIKENOD@vector?5too?5long?$AA@")]
	[DemangledName("\"vector too long\"")]
	[CleanName("String")]
	public static InlineArray_16_w3otm9c String_7b7kt9c
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_7b7kt9c.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_7b7kt9c.Value = value;
		}
	}

	[MangledName("??_7type_info@@6B@")]
	[DemangledName("const type_info::`vftable'")]
	public unsafe static void* _7type_info
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables._7type_info.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables._7type_info.Value = value;
		}
	}

	[MangledName("??_R0?AVbad_array_new_length@std@@@8")]
	[DemangledName("class std::bad_array_new_length `RTTI Type Descriptor'")]
	public static rtti_TypeDescriptor30 R0_AVbad_array_new_length
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R0_AVbad_array_new_length.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R0_AVbad_array_new_length.Value = value;
		}
	}

	[MangledName("__ImageBase")]
	[DemangledName("__ImageBase")]
	public static sbyte ImageBase
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.ImageBase.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.ImageBase.Value = value;
		}
	}

	[MangledName("_CT??_R0?AVbad_array_new_length@std@@@8??0bad_array_new_length@std@@QEAA@AEBV01@@Z24")]
	[DemangledName("_CT??_R0?AVbad_array_new_length@std@@@8??0bad_array_new_length@std@@QEAA@AEBV01@@Z24")]
	public static eh_CatchableType CT_R0_AVbad_array_new_length_std_8_0bad_array_new_length_std_QEAA_AEBV01_Z24
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.CT_R0_AVbad_array_new_length_std_8_0bad_array_new_length_std_QEAA_AEBV01_Z24.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.CT_R0_AVbad_array_new_length_std_8_0bad_array_new_length_std_QEAA_AEBV01_Z24.Value = value;
		}
	}

	[MangledName("??_R0?AVbad_alloc@std@@@8")]
	[DemangledName("class std::bad_alloc `RTTI Type Descriptor'")]
	public static rtti_TypeDescriptor19 R0_AVbad_alloc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R0_AVbad_alloc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R0_AVbad_alloc.Value = value;
		}
	}

	[MangledName("_CT??_R0?AVbad_alloc@std@@@8??0bad_alloc@std@@QEAA@AEBV01@@Z24")]
	[DemangledName("_CT??_R0?AVbad_alloc@std@@@8??0bad_alloc@std@@QEAA@AEBV01@@Z24")]
	public static eh_CatchableType CT_R0_AVbad_alloc_std_8_0bad_alloc_std_QEAA_AEBV01_Z24
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.CT_R0_AVbad_alloc_std_8_0bad_alloc_std_QEAA_AEBV01_Z24.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.CT_R0_AVbad_alloc_std_8_0bad_alloc_std_QEAA_AEBV01_Z24.Value = value;
		}
	}

	[MangledName("??_R0?AVexception@std@@@8")]
	[DemangledName("class std::exception `RTTI Type Descriptor'")]
	public static rtti_TypeDescriptor19 R0_AVexception
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R0_AVexception.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R0_AVexception.Value = value;
		}
	}

	[MangledName("_CT??_R0?AVexception@std@@@8??0exception@std@@QEAA@AEBV01@@Z24")]
	[DemangledName("_CT??_R0?AVexception@std@@@8??0exception@std@@QEAA@AEBV01@@Z24")]
	public static eh_CatchableType CT_R0_AVexception_std_8_0exception_std_QEAA_AEBV01_Z24
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.CT_R0_AVexception_std_8_0exception_std_QEAA_AEBV01_Z24.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.CT_R0_AVexception_std_8_0exception_std_QEAA_AEBV01_Z24.Value = value;
		}
	}

	[MangledName("_CTA3?AVbad_array_new_length@std@@")]
	[DemangledName("_CTA3?AVbad_array_new_length@std@@")]
	public static eh_CatchableTypeArray_3 CTA3_AVbad_array_new_length_std
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.CTA3_AVbad_array_new_length_std.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.CTA3_AVbad_array_new_length_std.Value = value;
		}
	}

	[MangledName("_TI3?AVbad_array_new_length@std@@")]
	[DemangledName("_TI3?AVbad_array_new_length@std@@")]
	public static eh_ThrowInfo TI3_AVbad_array_new_length_std
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.TI3_AVbad_array_new_length_std.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.TI3_AVbad_array_new_length_std.Value = value;
		}
	}

	[MangledName("??_C@_0BF@KINCDENJ@bad?5array?5new?5length?$AA@")]
	[DemangledName("\"bad array new length\"")]
	[CleanName("String")]
	public static InlineArray_21_w3otm9c String_gudorqc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_gudorqc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_gudorqc.Value = value;
		}
	}

	[MangledName("")]
	[CleanName("Variable")]
	public static Struct Variable_4yxjuqa
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.Variable_4yxjuqa.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.Variable_4yxjuqa.Value = value;
		}
	}

	[MangledName("??_R4bad_array_new_length@std@@6B@")]
	[DemangledName("const std::bad_array_new_length::`RTTI Complete Object Locator'")]
	public static rtti_CompleteObjectLocator R4bad_array_new_length
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R4bad_array_new_length.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R4bad_array_new_length.Value = value;
		}
	}

	[MangledName("??_R3bad_array_new_length@std@@8")]
	[DemangledName("std::bad_array_new_length::`RTTI Class Hierarchy Descriptor'")]
	public static rtti_ClassHierarchyDescriptor R3bad_array_new_length
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R3bad_array_new_length.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R3bad_array_new_length.Value = value;
		}
	}

	[MangledName("??_R2bad_array_new_length@std@@8")]
	[DemangledName("std::bad_array_new_length::`RTTI Base Class Array'")]
	public static InlineArray_4_vqkfj7a R2bad_array_new_length
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R2bad_array_new_length.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R2bad_array_new_length.Value = value;
		}
	}

	[MangledName("??_R1A@?0A@EA@bad_array_new_length@std@@8")]
	[DemangledName("std::bad_array_new_length::`RTTI Base Class Descriptor at (0, -1, 0, 64)'")]
	[CleanName("R1A")]
	public static rtti_BaseClassDescriptor R1A_9w7p7zd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R1A_9w7p7zd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R1A_9w7p7zd.Value = value;
		}
	}

	[MangledName("??_R1A@?0A@EA@bad_alloc@std@@8")]
	[DemangledName("std::bad_alloc::`RTTI Base Class Descriptor at (0, -1, 0, 64)'")]
	[CleanName("R1A")]
	public static rtti_BaseClassDescriptor R1A_vzsilob
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R1A_vzsilob.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R1A_vzsilob.Value = value;
		}
	}

	[MangledName("??_R3bad_alloc@std@@8")]
	[DemangledName("std::bad_alloc::`RTTI Class Hierarchy Descriptor'")]
	public static rtti_ClassHierarchyDescriptor R3bad_alloc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R3bad_alloc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R3bad_alloc.Value = value;
		}
	}

	[MangledName("??_R2bad_alloc@std@@8")]
	[DemangledName("std::bad_alloc::`RTTI Base Class Array'")]
	public static InlineArray_3_vqkfj7a R2bad_alloc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R2bad_alloc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R2bad_alloc.Value = value;
		}
	}

	[MangledName("??_R1A@?0A@EA@exception@std@@8")]
	[DemangledName("std::exception::`RTTI Base Class Descriptor at (0, -1, 0, 64)'")]
	[CleanName("R1A")]
	public static rtti_BaseClassDescriptor R1A_bbqer2a
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R1A_bbqer2a.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R1A_bbqer2a.Value = value;
		}
	}

	[MangledName("??_R3exception@std@@8")]
	[DemangledName("std::exception::`RTTI Class Hierarchy Descriptor'")]
	public static rtti_ClassHierarchyDescriptor R3exception
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R3exception.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R3exception.Value = value;
		}
	}

	[MangledName("??_R2exception@std@@8")]
	[DemangledName("std::exception::`RTTI Base Class Array'")]
	public static InlineArray_2_vqkfj7a R2exception
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R2exception.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R2exception.Value = value;
		}
	}

	[MangledName("")]
	[CleanName("Variable")]
	public static Struct Variable_zdor9mc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.Variable_zdor9mc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.Variable_zdor9mc.Value = value;
		}
	}

	[MangledName("??_R4bad_alloc@std@@6B@")]
	[DemangledName("const std::bad_alloc::`RTTI Complete Object Locator'")]
	public static rtti_CompleteObjectLocator R4bad_alloc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R4bad_alloc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R4bad_alloc.Value = value;
		}
	}

	[MangledName("")]
	[CleanName("Variable")]
	public static Struct Variable_x4f2ufc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.Variable_x4f2ufc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.Variable_x4f2ufc.Value = value;
		}
	}

	[MangledName("??_R4exception@std@@6B@")]
	[DemangledName("const std::exception::`RTTI Complete Object Locator'")]
	public static rtti_CompleteObjectLocator R4exception
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.R4exception.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.R4exception.Value = value;
		}
	}

	[MangledName("??_C@_0BC@EOODALEL@Unknown?5exception?$AA@")]
	[DemangledName("\"Unknown exception\"")]
	[CleanName("String")]
	public static InlineArray_18_w3otm9c String_ja3ma5d
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_ja3ma5d.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_ja3ma5d.Value = value;
		}
	}

	[MangledName("?g_defl_packed_code_size_syms_swizzle@fpng@@3PAEA")]
	[DemangledName("unsigned char *fpng::g_defl_packed_code_size_syms_swizzle")]
	public static InlineArray_19_w3otm9c g_defl_packed_code_size_syms_swizzle
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_defl_packed_code_size_syms_swizzle.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_defl_packed_code_size_syms_swizzle.Value = value;
		}
	}

	[MangledName("??_C@_1DO@PIFJFOL@?$AAc?$AAo?$AAd?$AAe?$AA?5?$AA?$DM?$AA?5?$AAD?$AAE?$AAF?$AAL?$AA_?$AAM?$AAA?$AAX?$AA_?$AAH?$AAU?$AAF?$AAF?$AA_?$AAS?$AAY?$AAM?$AAB?$AAO?$AAL?$AAS?$AA_?$AA2?$AA?$AA@")]
	[DemangledName("L\"code < DEFL_MAX_HUFF_SYMBOLS_2\"")]
	[CleanName("String")]
	public static InlineArray_31_oeds4ea String_erkdsbd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_erkdsbd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_erkdsbd.Value = value;
		}
	}

	[MangledName("??_C@_03DJLFMAPG@?$AC?$AD?$AH?$AA@")]
	[DemangledName("\"\\x02\\x03\\a\"")]
	[CleanName("String")]
	public static InlineArray_4_w3otm9c String_o9ulvgb
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_o9ulvgb.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_o9ulvgb.Value = value;
		}
	}

	[MangledName("?g_dyn_huff_3@fpng@@3QBEB")]
	[DemangledName("unsigned char const *const fpng::g_dyn_huff_3")]
	public static InlineArray_62_w3otm9c g_dyn_huff_3
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_3.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_3.Value = value;
		}
	}

	[MangledName("?g_dyn_huff_3_codes@fpng@@3QBU<unnamed-type-g_dyn_huff_3_codes>@1@B")]
	[DemangledName("struct fpng::<unnamed-type-g_dyn_huff_3_codes> const *const fpng::g_dyn_huff_3_codes")]
	public static InlineArray_288_chfa5zb g_dyn_huff_3_codes
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_3_codes.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_3_codes.Value = value;
		}
	}

	[MangledName("?g_dyn_huff_4@fpng@@3QBEB")]
	[DemangledName("unsigned char const *const fpng::g_dyn_huff_4")]
	public static InlineArray_61_w3otm9c g_dyn_huff_4
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_4.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_4.Value = value;
		}
	}

	[MangledName("?g_dyn_huff_4_codes@fpng@@3QBU<unnamed-type-g_dyn_huff_4_codes>@1@B")]
	[DemangledName("struct fpng::<unnamed-type-g_dyn_huff_4_codes> const *const fpng::g_dyn_huff_4_codes")]
	public static InlineArray_288_mcnvamb g_dyn_huff_4_codes
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_4_codes.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_dyn_huff_4_codes.Value = value;
		}
	}

	[MangledName("?s_png_sig@?1??fpng_get_info_internal@fpng@@YAHPEBXIAEAI1111@Z@4QBEB")]
	[DemangledName("unsigned char const *const `int __cdecl fpng::fpng_get_info_internal(void const *, unsigned int, unsigned int &, unsigned int &, unsigned int &, unsigned int &, unsigned int &)'::`2'::s_png_sig")]
	public static InlineArray_8_w3otm9c s_png_sig
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.s_png_sig.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.s_png_sig.Value = value;
		}
	}

	[MangledName("??_C@_04NHMHICBH@IDAT?$AA@")]
	[DemangledName("\"IDAT\"")]
	[CleanName("String")]
	public static InlineArray_5_w3otm9c String_qjc9xdd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_qjc9xdd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_qjc9xdd.Value = value;
		}
	}

	[MangledName("??_C@_04COOFLABO@IEND?$AA@")]
	[DemangledName("\"IEND\"")]
	[CleanName("String")]
	public static InlineArray_5_w3otm9c String_p7olgfc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_p7olgfc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_p7olgfc.Value = value;
		}
	}

	[MangledName("??_C@_04DGOOLJLG@fdEC?$AA@")]
	[DemangledName("\"fdEC\"")]
	[CleanName("String")]
	public static InlineArray_5_w3otm9c String_22lii4a
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_22lii4a.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_22lii4a.Value = value;
		}
	}

	[MangledName("??_C@_1DE@LOJFIBNB@?$AAs?$AAr?$AAc?$AA_?$AAl?$AAe?$AAn?$AA?5?$AA?$DO?$AA?$DN?$AA?5?$AA?$CI?$AAz?$AAl?$AAi?$AAb?$AA_?$AAl?$AAe?$AAn?$AA?5?$AA?$CL?$AA?5?$AA4?$AA?$CJ?$AA?$AA@")]
	[DemangledName("L\"src_len >= (zlib_len + 4)\"")]
	[CleanName("String")]
	public static InlineArray_26_oeds4ea String_497patd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_497patd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_497patd.Value = value;
		}
	}

	[MangledName("??_C@_1BO@DPMPJCNO@?$AAl?$AA?5?$AA?$CG?$AA?$CG?$AA?5?$AA?$CI?$AAl?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA3?$AA2?$AA?$CJ?$AA?$AA@")]
	[DemangledName("L\"l && (l <= 32)\"")]
	[CleanName("String")]
	public static InlineArray_15_oeds4ea String_aa3exrc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_aa3exrc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_aa3exrc.Value = value;
		}
	}

	[MangledName("??_C@_1FA@PEENLMBD@?$AAb?$AAi?$AAt?$AA_?$AAb?$AAu?$AAf?$AA_?$AAs?$AAi?$AAz?$AAe?$AA?5?$AA?$DO?$AA?$DN?$AA?5?$AAF?$AAP?$AAN?$AAG?$AA_?$AAD?$AAE?$AAC?$AAO?$AAD?$AAE?$AAR?$AA_?$AAT?$AAA?$AAB@")]
	[DemangledName("L\"bit_buf_size >= FPNG_DECODER_TAB\"...")]
	[CleanName("String")]
	public static InlineArray_40_oeds4ea String_ktv524d
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_ktv524d.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_ktv524d.Value = value;
		}
	}

	[MangledName("??_C@_1BA@ONNHMHEP@?$AAl?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA3?$AA2?$AA?$AA@")]
	[DemangledName("L\"l <= 32\"")]
	[CleanName("String")]
	public static InlineArray_8_oeds4ea String_mf9hyza
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_mf9hyza.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_mf9hyza.Value = value;
		}
	}

	[MangledName("?s_length_range@fpng@@3QBHB")]
	[DemangledName("int const *const fpng::s_length_range")]
	public static InlineArray_31_vqkfj7a s_length_range
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.s_length_range.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.s_length_range.Value = value;
		}
	}

	[MangledName("?s_length_extra@fpng@@3QBHB")]
	[DemangledName("int const *const fpng::s_length_extra")]
	public static InlineArray_31_vqkfj7a s_length_extra
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.s_length_extra.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.s_length_extra.Value = value;
		}
	}

	[MangledName("??_C@_1EM@LNFOKHHI@?$AAl?$AA?5?$AA?$CG?$AA?$CG?$AA?5?$AA?$CI?$AAl?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA3?$AA2?$AA?$CJ?$AA?5?$AA?$CG?$AA?$CG?$AA?5?$AA?$CI?$AAb?$AAi?$AAt?$AA_?$AAb?$AAu?$AAf?$AA_?$AAs?$AAi?$AAz?$AAe?$AA?5@")]
	[DemangledName("L\"l && (l <= 32) && (bit_buf_size \"...")]
	[CleanName("String")]
	public static InlineArray_38_oeds4ea String_ftb25bb
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_ftb25bb.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_ftb25bb.Value = value;
		}
	}

	[MangledName("??_C@_1DO@GINGNLCO@?$AAl?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AA3?$AA2?$AA?5?$AA?$CG?$AA?$CG?$AA?5?$AA?$CI?$AAb?$AAi?$AAt?$AA_?$AAb?$AAu?$AAf?$AA_?$AAs?$AAi?$AAz?$AAe?$AA?5?$AA?$DO?$AA?$DN?$AA?5?$AAl?$AA?$CJ?$AA?$AA@")]
	[DemangledName("L\"l <= 32 && (bit_buf_size >= l)\"")]
	[CleanName("String")]
	public static InlineArray_31_oeds4ea String_bahgbqd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_bahgbqd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_bahgbqd.Value = value;
		}
	}

	[MangledName("??_C@_1CG@ELPAFFDG@?$AA?$CI?$AAr?$AAu?$AAn?$AA_?$AAl?$AAe?$AAn?$AA?5?$AA?$CF?$AA?5?$AA3?$AA?$CJ?$AA?5?$AA?$DN?$AA?$DN?$AA?5?$AA0?$AA?$AA@")]
	[DemangledName("L\"(run_len % 3) == 0\"")]
	[CleanName("String")]
	public static InlineArray_19_oeds4ea String_a5rp7bb
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_a5rp7bb.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_a5rp7bb.Value = value;
		}
	}

	[MangledName("?g_run_len3_to_4@fpng@@3QBGB")]
	[DemangledName("unsigned short const *const fpng::g_run_len3_to_4")]
	public static InlineArray_259_oeds4ea g_run_len3_to_4
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.g_run_len3_to_4.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.g_run_len3_to_4.Value = value;
		}
	}

	[MangledName("??_C@_1EK@NPDOIFAN@?$AA?$CI?$AAs?$AAr?$AAc?$AA_?$AAc?$AAh?$AAa?$AAn?$AAs?$AA?5?$AA?$DN?$AA?$DN?$AA?5?$AA3?$AA?$CJ?$AA?5?$AA?$HM?$AA?$HM?$AA?5?$AA?$CI?$AAs?$AAr?$AAc?$AA_?$AAc?$AAh?$AAa?$AAn?$AAs?$AA?5?$AA?$DN@")]
	[DemangledName("L\"(src_chans == 3) || (src_chans =\"...")]
	[CleanName("String")]
	public static InlineArray_37_oeds4ea String_zixkhbc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_zixkhbc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_zixkhbc.Value = value;
		}
	}

	[MangledName("??_C@_1EK@BJMGPLPL@?$AA?$CI?$AAd?$AAs?$AAt?$AA_?$AAc?$AAh?$AAa?$AAn?$AAs?$AA?5?$AA?$DN?$AA?$DN?$AA?5?$AA3?$AA?$CJ?$AA?5?$AA?$HM?$AA?$HM?$AA?5?$AA?$CI?$AAd?$AAs?$AAt?$AA_?$AAc?$AAh?$AAa?$AAn?$AAs?$AA?5?$AA?$DN@")]
	[DemangledName("L\"(dst_chans == 3) || (dst_chans =\"...")]
	[CleanName("String")]
	public static InlineArray_37_oeds4ea String_kppxlqc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_kppxlqc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_kppxlqc.Value = value;
		}
	}

	[MangledName("??_C@_1BE@MMMJACNB@?$AA?$CB?$AAc?$AAo?$AAm?$AAp?$AA_?$AAo?$AAf?$AAs?$AA?$AA@")]
	[DemangledName("L\"!comp_ofs\"")]
	[CleanName("String")]
	public static InlineArray_10_oeds4ea String_ji2h3xd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_ji2h3xd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_ji2h3xd.Value = value;
		}
	}

	[MangledName("?s_bit_length_order@?1??prepare_dynamic_block@fpng@@YA_NPEBEIAEAI1AEA_KPEAII@Z@4QBEB")]
	[DemangledName("unsigned char const *const `bool __cdecl fpng::prepare_dynamic_block(unsigned char const *, unsigned int, unsigned int &, unsigned int &, unsigned __int64 &, unsigned int *, unsigned int)'::`2'::s_bit_length_order")]
	public static InlineArray_19_w3otm9c s_bit_length_order
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.s_bit_length_order.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.s_bit_length_order.Value = value;
		}
	}

	[MangledName("??_C@_1EI@CMIKALPF@?$AAs?$AAy?$AAm?$AA_?$AAb?$AAi?$AAt?$AAs?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AAF?$AAP?$AAN?$AAG?$AA_?$AAD?$AAE?$AAC?$AAO?$AAD?$AAE?$AAR?$AA_?$AAT?$AAA?$AAB?$AAL?$AAE?$AA_?$AAB@")]
	[DemangledName("L\"sym_bits <= FPNG_DECODER_TABLE_B\"...")]
	[CleanName("String")]
	public static InlineArray_36_oeds4ea String_nlshwsc
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_nlshwsc.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_nlshwsc.Value = value;
		}
	}

	[MangledName("??_C@_1FE@PPLLBMKL@?$AAp?$AAC?$AAo?$AAd?$AAe?$AA_?$AAs?$AAi?$AAz?$AAe?$AAs?$AA?$FL?$AAi?$AA?$FN?$AA?5?$AA?$DM?$AA?$DN?$AA?5?$AAF?$AAP?$AAN?$AAG?$AA_?$AAD?$AAE?$AAC?$AAO?$AAD?$AAE?$AAR?$AA_?$AAT@")]
	[DemangledName("L\"pCode_sizes[i] <= FPNG_DECODER_T\"...")]
	[CleanName("String")]
	public static InlineArray_42_oeds4ea String_32otbnd
	{
		get
		{
			return AssetRipper.Conversions.FastPng.GlobalVariables.String_32otbnd.Value;
		}
		set
		{
			AssetRipper.Conversions.FastPng.GlobalVariables.String_32otbnd.Value = value;
		}
	}

	[MangledName("?fpng_init@fpng@@YAXXZ")]
	[DemangledName("void __cdecl fpng::fpng_init(void)")]
	public static void fpng_init()
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_init.Invoke();
	}

	[MangledName("?fpng_cpu_supports_sse41@fpng@@YA_NXZ")]
	[DemangledName("bool __cdecl fpng::fpng_cpu_supports_sse41(void)")]
	public static bool fpng_cpu_supports_sse41()
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_cpu_supports_sse41.Invoke();
	}

	[MangledName("?fpng_crc32@fpng@@YAIPEBX_KI@Z")]
	[DemangledName("unsigned int __cdecl fpng::fpng_crc32(void const *, unsigned __int64, unsigned int)")]
	public unsafe static int fpng_crc32(void* pData, long size, int prev_crc32)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_crc32.Invoke(pData, size, prev_crc32);
	}

	[MangledName("?crc32_slice_by_4@fpng@@YAIPEBX_KI@Z")]
	[DemangledName("unsigned int __cdecl fpng::crc32_slice_by_4(void const *, unsigned __int64, unsigned int)")]
	public unsafe static int crc32_slice_by_4(void* pData, long data_len, int cur_crc32)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.crc32_slice_by_4.Invoke(pData, data_len, cur_crc32);
	}

	[MangledName("?fpng_adler32@fpng@@YAIPEBX_KI@Z")]
	[DemangledName("unsigned int __cdecl fpng::fpng_adler32(void const *, unsigned __int64, unsigned int)")]
	public unsafe static int fpng_adler32(void* pData, long size, int adler)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_adler32.Invoke(pData, size, adler);
	}

	[MangledName("?fpng_adler32_scalar@fpng@@YAIPEBE_KI@Z")]
	[DemangledName("unsigned int __cdecl fpng::fpng_adler32_scalar(unsigned char const *, unsigned __int64, unsigned int)")]
	public unsafe static int fpng_adler32_scalar(void* ptr, long buf_len, int adler)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_adler32_scalar.Invoke(ptr, buf_len, adler);
	}

	[MangledName("?fpng_encode_image_to_memory@fpng@@YA_NPEBXIIIAEAV?$vector@EV?$allocator@E@std@@@std@@I@Z")]
	[DemangledName("bool __cdecl fpng::fpng_encode_image_to_memory(void const *, unsigned int, unsigned int, unsigned int, class std::vector<unsigned char, class std::allocator<unsigned char>> &, unsigned int)")]
	public unsafe static bool fpng_encode_image_to_memory(void* pImage, int w, int h, int num_chans, void* out_buf, int flags)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_encode_image_to_memory.Invoke(pImage, w, h, num_chans, out_buf, flags);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?endian_check@fpng@@YA_NXZ")]
	[DemangledName("bool __cdecl fpng::endian_check(void)")]
	public static bool endian_check()
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.endian_check.Invoke();
	}

	[MangledName("_wassert")]
	[DemangledName("_wassert")]
	public unsafe static void wassert(void* parameter_0, void* parameter_1, int parameter_2)
	{
		try
		{
			AssetRipper.Conversions.FastPng.Intrinsics.Implemented.wassert.Invoke(parameter_0, parameter_1, parameter_2);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??0?$vector@EV?$allocator@E@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::vector<unsigned char, class std::allocator<unsigned char>>(void)")]
	public unsafe static void* vector_unsigned_char_class_std_allocator_unsigned_char_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_char_class_std_allocator_unsigned_char_Constructor.Invoke(@this);
	}

	[MangledName("?resize@?$vector@EV?$allocator@E@std@@@std@@QEAAX_K@Z")]
	[DemangledName("public: void __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::resize(unsigned __int64)")]
	[CleanName("resize")]
	public unsafe static void resize_wwmwtsa(void* @this, long Newsize)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.resize_wwmwtsa.Invoke(@this, Newsize);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("__CxxFrameHandler3")]
	[DemangledName("__CxxFrameHandler3")]
	public static int CxxFrameHandler3(ReadOnlySpan<nint> args)
	{
		return AssetRipper.Conversions.FastPng.Intrinsics.Implemented.CxxFrameHandler3.Invoke(args);
	}

	[MangledName("??A?$vector@EV?$allocator@E@std@@@std@@QEAAAEAE_K@Z")]
	[DemangledName("public: unsigned char & __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::operator[](unsigned __int64)")]
	public unsafe static void* vector_unsigned_char_class_std_allocator_unsigned_char_Index(void* @this, long Pos)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_char_class_std_allocator_unsigned_char_Index.Invoke(@this, Pos);
	}

	[MangledName("?apply_filter@fpng@@YAXIHHIIPEBE0PEAE@Z")]
	[DemangledName("void __cdecl fpng::apply_filter(unsigned int, int, int, unsigned int, unsigned int, unsigned char const *, unsigned char const *, unsigned char *)")]
	public unsafe static void apply_filter(int filter, int w, int h, int num_chans, int bpl, void* pSrc, void* pPrev_src, void* pDst)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.apply_filter.Invoke(filter, w, h, num_chans, bpl, pSrc, pPrev_src, pDst);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("?pixel_deflate_dyn_3_rle@fpng@@YAIPEBEIIPEAEI@Z")]
	[DemangledName("unsigned int __cdecl fpng::pixel_deflate_dyn_3_rle(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int)")]
	public unsafe static int pixel_deflate_dyn_3_rle(void* pImg, int w, int h, void* pDst, int dst_buf_size)
	{
		int result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.pixel_deflate_dyn_3_rle.Invoke(pImg, w, h, pDst, dst_buf_size);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?size@?$vector@EV?$allocator@E@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::size(void) const")]
	[CleanName("size")]
	public unsafe static long size_4io4p4d(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.size_4io4p4d.Invoke(@this);
	}

	[MangledName("?data@?$vector@EV?$allocator@E@std@@@std@@QEAAPEAEXZ")]
	[DemangledName("public: unsigned char * __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::data(void)")]
	[CleanName("data")]
	public unsafe static void* data_efhnzgd(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.data_efhnzgd.Invoke(@this);
	}

	[MangledName("?pixel_deflate_dyn_3_rle_one_pass@fpng@@YAIPEBEIIPEAEI@Z")]
	[DemangledName("unsigned int __cdecl fpng::pixel_deflate_dyn_3_rle_one_pass(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int)")]
	public unsafe static int pixel_deflate_dyn_3_rle_one_pass(void* pImg, int w, int h, void* pDst, int dst_buf_size)
	{
		int result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.pixel_deflate_dyn_3_rle_one_pass.Invoke(pImg, w, h, pDst, dst_buf_size);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?pixel_deflate_dyn_4_rle@fpng@@YAIPEBEIIPEAEI@Z")]
	[DemangledName("unsigned int __cdecl fpng::pixel_deflate_dyn_4_rle(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int)")]
	public unsafe static int pixel_deflate_dyn_4_rle(void* pImg, int w, int h, void* pDst, int dst_buf_size)
	{
		int result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.pixel_deflate_dyn_4_rle.Invoke(pImg, w, h, pDst, dst_buf_size);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?pixel_deflate_dyn_4_rle_one_pass@fpng@@YAIPEBEIIPEAEI@Z")]
	[DemangledName("unsigned int __cdecl fpng::pixel_deflate_dyn_4_rle_one_pass(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int)")]
	public unsafe static int pixel_deflate_dyn_4_rle_one_pass(void* pImg, int w, int h, void* pDst, int dst_buf_size)
	{
		int result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.pixel_deflate_dyn_4_rle_one_pass.Invoke(pImg, w, h, pDst, dst_buf_size);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?write_raw_block@fpng@@YAIPEBEIPEAEI@Z")]
	[DemangledName("unsigned int __cdecl fpng::write_raw_block(unsigned char const *, unsigned int, unsigned char *, unsigned int)")]
	public unsafe static int write_raw_block(void* pSrc, int src_len, void* pDst, int dst_buf_size)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.write_raw_block.Invoke(pSrc, src_len, pDst, dst_buf_size);
	}

	[MangledName("llvm.memcpy.p0.p0.i64")]
	[DemangledName("llvm.memcpy.p0.p0.i64")]
	public unsafe static void llvm_memcpy_p0_p0_i64(void* parameter_0, void* parameter_1, long parameter_2, bool parameter_3)
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Implemented.llvm_memcpy_p0_p0_i64.Invoke(parameter_0, parameter_1, parameter_2, parameter_3);
	}

	[MangledName("?vector_append@fpng@@YAXAEAV?$vector@EV?$allocator@E@std@@@std@@PEBX_K@Z")]
	[DemangledName("void __cdecl fpng::vector_append(class std::vector<unsigned char, class std::allocator<unsigned char>> &, void const *, unsigned __int64)")]
	public unsafe static void vector_append(void* buf, void* pData, long len)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.vector_append.Invoke(buf, pData, len);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??1?$vector@EV?$allocator@E@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::~vector<unsigned char, class std::allocator<unsigned char>>(void)")]
	public unsafe static void vector_unsigned_char_class_std_allocator_unsigned_char_Destructor(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_char_class_std_allocator_unsigned_char_Destructor.Invoke(@this);
	}

	[MangledName("?fpng_get_info@fpng@@YAHPEBXIAEAI11@Z")]
	[DemangledName("int __cdecl fpng::fpng_get_info(void const *, unsigned int, unsigned int &, unsigned int &, unsigned int &)")]
	public unsafe static int fpng_get_info(void* pImage, int image_size, void* width, void* height, void* channels_in_file)
	{
		int result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_get_info.Invoke(pImage, image_size, width, height, channels_in_file);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?fpng_get_info_internal@fpng@@YAHPEBXIAEAI1111@Z")]
	[DemangledName("int __cdecl fpng::fpng_get_info_internal(void const *, unsigned int, unsigned int &, unsigned int &, unsigned int &, unsigned int &, unsigned int &)")]
	public unsafe static int fpng_get_info_internal(void* pImage, int image_size, void* width, void* height, void* channels_in_file, void* idat_ofs, void* idat_len)
	{
		int result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_get_info_internal.Invoke(pImage, image_size, width, height, channels_in_file, idat_ofs, idat_len);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?fpng_decode_memory@fpng@@YAHPEBXIAEAV?$vector@EV?$allocator@E@std@@@std@@AEAI22I@Z")]
	[DemangledName("int __cdecl fpng::fpng_decode_memory(void const *, unsigned int, class std::vector<unsigned char, class std::allocator<unsigned char>> &, unsigned int &, unsigned int &, unsigned int &, unsigned int)")]
	public unsafe static int fpng_decode_memory(void* pImage, int image_size, void* @out, void* width, void* height, void* channels_in_file, int desired_channels)
	{
		int result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_decode_memory.Invoke(pImage, image_size, @out, width, height, channels_in_file, desired_channels);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$fpng_pixel_zlib_decompress_3@$02@fpng@@YA_NPEBEIIPEAEII@Z")]
	[DemangledName("bool __cdecl fpng::fpng_pixel_zlib_decompress_3<3>(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int, unsigned int)")]
	[CleanName("fpng_pixel_zlib_decompress_3")]
	public unsafe static bool fpng_pixel_zlib_decompress_3_prmnnjb(void* pSrc, int src_len, int zlib_len, void* pDst, int w, int h)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_pixel_zlib_decompress_3_prmnnjb.Invoke(pSrc, src_len, zlib_len, pDst, w, h);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$fpng_pixel_zlib_decompress_4@$02@fpng@@YA_NPEBEIIPEAEII@Z")]
	[DemangledName("bool __cdecl fpng::fpng_pixel_zlib_decompress_4<3>(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int, unsigned int)")]
	[CleanName("fpng_pixel_zlib_decompress_4")]
	public unsafe static bool fpng_pixel_zlib_decompress_4_xygi9td(void* pSrc, int src_len, int zlib_len, void* pDst, int w, int h)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_pixel_zlib_decompress_4_xygi9td.Invoke(pSrc, src_len, zlib_len, pDst, w, h);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$fpng_pixel_zlib_decompress_3@$03@fpng@@YA_NPEBEIIPEAEII@Z")]
	[DemangledName("bool __cdecl fpng::fpng_pixel_zlib_decompress_3<4>(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int, unsigned int)")]
	[CleanName("fpng_pixel_zlib_decompress_3")]
	public unsafe static bool fpng_pixel_zlib_decompress_3_yffw42c(void* pSrc, int src_len, int zlib_len, void* pDst, int w, int h)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_pixel_zlib_decompress_3_yffw42c.Invoke(pSrc, src_len, zlib_len, pDst, w, h);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$fpng_pixel_zlib_decompress_4@$03@fpng@@YA_NPEBEIIPEAEII@Z")]
	[DemangledName("bool __cdecl fpng::fpng_pixel_zlib_decompress_4<4>(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int, unsigned int)")]
	[CleanName("fpng_pixel_zlib_decompress_4")]
	public unsafe static bool fpng_pixel_zlib_decompress_4_amptoaa(void* pSrc, int src_len, int zlib_len, void* pDst, int w, int h)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_pixel_zlib_decompress_4_amptoaa.Invoke(pSrc, src_len, zlib_len, pDst, w, h);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??0?$allocator@I@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::allocator<unsigned int>::allocator<unsigned int>(void)")]
	public unsafe static void* allocator_unsigned_int_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.allocator_unsigned_int_Constructor.Invoke(@this);
	}

	[MangledName("??0?$vector@IV?$allocator@I@std@@@std@@QEAA@_KAEBV?$allocator@I@1@@Z")]
	[DemangledName("public: __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::vector<unsigned int, class std::allocator<unsigned int>>(unsigned __int64, class std::allocator<unsigned int> const &)")]
	public unsafe static void* vector_unsigned_int_class_std_allocator_unsigned_int_Constructor(void* @this, long Count, void* Al)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_int_class_std_allocator_unsigned_int_Constructor.Invoke(@this, Count, Al);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?data@?$vector@IV?$allocator@I@std@@@std@@QEAAPEAIXZ")]
	[DemangledName("public: unsigned int * __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::data(void)")]
	[CleanName("data")]
	public unsafe static void* data_uf9yj3b(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.data_uf9yj3b.Invoke(@this);
	}

	[MangledName("llvm.memset.p0.i64")]
	[DemangledName("llvm.memset.p0.i64")]
	public unsafe static void llvm_memset_p0_i64(void* parameter_0, sbyte parameter_1, long parameter_2, bool parameter_3)
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Implemented.llvm_memset_p0_i64.Invoke(parameter_0, parameter_1, parameter_2, parameter_3);
	}

	[MangledName("?READ_RGB_PIXEL@fpng@@YAIPEBX@Z")]
	[DemangledName("unsigned int __cdecl fpng::READ_RGB_PIXEL(void const *)")]
	public unsafe static int READ_RGB_PIXEL(void* p)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.READ_RGB_PIXEL.Invoke(p);
	}

	[MangledName("??$minimum@H@fpng@@YAHHH@Z")]
	[DemangledName("int __cdecl fpng::minimum<int>(int, int)")]
	[CleanName("minimum")]
	public static int minimum_ymfg3kb(int a, int b)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.minimum_ymfg3kb.Invoke(a, b);
	}

	[MangledName("?size@?$vector@IV?$allocator@I@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::size(void) const")]
	[CleanName("size")]
	public unsafe static long size_wtrvsgc(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.size_wtrvsgc.Invoke(@this);
	}

	[MangledName("?adjust_freq32@fpng@@YAXIPEAIPEAG@Z")]
	[DemangledName("void __cdecl fpng::adjust_freq32(unsigned int, unsigned int *, unsigned short *)")]
	public unsafe static void adjust_freq32(int num_freq, void* pFreq, void* pFreq16)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.adjust_freq32.Invoke(num_freq, pFreq, pFreq16);
	}

	[MangledName("?defl_start_dynamic_block@fpng@@YA_NPEAUdefl_huff@1@PEAEAEAIIAEA_KAEAH@Z")]
	[DemangledName("bool __cdecl fpng::defl_start_dynamic_block(struct fpng::defl_huff *, unsigned char *, unsigned int &, unsigned int, unsigned __int64 &, int &)")]
	public unsafe static bool defl_start_dynamic_block(void* d, void* pDst, void* dst_ofs, int dst_buf_size, void* bit_buf, void* bit_buf_size)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.defl_start_dynamic_block.Invoke(d, pDst, dst_ofs, dst_buf_size, bit_buf, bit_buf_size);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??A?$vector@IV?$allocator@I@std@@@std@@QEAAAEAI_K@Z")]
	[DemangledName("public: unsigned int & __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::operator[](unsigned __int64)")]
	public unsafe static void* vector_unsigned_int_class_std_allocator_unsigned_int_Index(void* @this, long Pos)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_int_class_std_allocator_unsigned_int_Index.Invoke(@this, Pos);
	}

	[MangledName("??1?$vector@IV?$allocator@I@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::~vector<unsigned int, class std::allocator<unsigned int>>(void)")]
	public unsafe static void vector_unsigned_int_class_std_allocator_unsigned_int_Destructor(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_int_class_std_allocator_unsigned_int_Destructor.Invoke(@this);
	}

	[MangledName("??$?0AEBV?$allocator@I@std@@$$V@?$_Compressed_pair@V?$allocator@I@std@@V?$_Vector_val@U?$_Simple_types@I@std@@@2@$00@std@@QEAA@U_One_then_variadic_args_t@1@AEBV?$allocator@I@1@@Z")]
	[DemangledName("public: __cdecl std::_Compressed_pair<class std::allocator<unsigned int>, class std::_Vector_val<struct std::_Simple_types<unsigned int>>, 1>::_Compressed_pair<class std::allocator<unsigned int>, class std::_Vector_val<struct std::_Simple_types<unsigned int>>, 1><class std::allocator<unsigned int> const &>(struct std::_One_then_variadic_args_t, class std::allocator<unsigned int> const &)")]
	public unsafe static void* Compressed_pair_class_std_allocator_unsigned_int_class_std_Vector_val_struct_std_Simple_types_unsigned_int_1_Constructor(void* @this, sbyte coerce, void* Val1)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Compressed_pair_class_std_allocator_unsigned_int_class_std_Vector_val_struct_std_Simple_types_unsigned_int_1_Constructor.Invoke(@this, coerce, Val1);
	}

	[MangledName("??$_Construct_n@$$V@?$vector@IV?$allocator@I@std@@@std@@AEAAX_K@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::_Construct_n<>(unsigned __int64)")]
	public unsafe static void Construct_n(void* @this, long Count)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Construct_n.Invoke(@this, Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??0?$_Vector_val@U?$_Simple_types@I@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::_Vector_val<struct std::_Simple_types<unsigned int>>::_Vector_val<struct std::_Simple_types<unsigned int>>(void)")]
	public unsafe static void* Vector_val_struct_std_Simple_types_unsigned_int_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Vector_val_struct_std_Simple_types_unsigned_int_Constructor.Invoke(@this);
	}

	[MangledName("?_Getal@?$vector@IV?$allocator@I@std@@@std@@AEAAAEAV?$allocator@I@2@XZ")]
	[DemangledName("private: class std::allocator<unsigned int> & __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::_Getal(void)")]
	[CleanName("Getal")]
	public unsafe static void* Getal_t2pmqoc(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Getal_t2pmqoc.Invoke(@this);
	}

	[MangledName("??$_Get_proxy_allocator@V?$allocator@I@std@@@std@@YA?AU_Fake_allocator@0@AEBV?$allocator@I@0@@Z")]
	[DemangledName("struct std::_Fake_allocator __cdecl std::_Get_proxy_allocator<class std::allocator<unsigned int>>(class std::allocator<unsigned int> const &)")]
	[CleanName("Get_proxy_allocator")]
	public unsafe static sbyte Get_proxy_allocator_uag3hnc(void* parameter_0)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_proxy_allocator_uag3hnc.Invoke(parameter_0);
	}

	[MangledName("??0_Fake_proxy_ptr_impl@std@@QEAA@AEBU_Fake_allocator@1@AEBU_Container_base0@1@@Z")]
	[DemangledName("public: __cdecl std::_Fake_proxy_ptr_impl::_Fake_proxy_ptr_impl(struct std::_Fake_allocator const &, struct std::_Container_base0 const &)")]
	public unsafe static void* Fake_proxy_ptr_impl_Constructor(void* @this, void* parameter_1, void* parameter_2)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Fake_proxy_ptr_impl_Constructor.Invoke(@this, parameter_1, parameter_2);
	}

	[MangledName("?_Buy_nonzero@?$vector@IV?$allocator@I@std@@@std@@AEAAX_K@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::_Buy_nonzero(unsigned __int64)")]
	public unsafe static void Buy_nonzero(void* @this, long Newcapacity)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Buy_nonzero.Invoke(@this, Newcapacity);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??$_Uninitialized_value_construct_n@V?$allocator@I@std@@@std@@YAPEAIPEAI_KAEAV?$allocator@I@0@@Z")]
	[DemangledName("unsigned int * __cdecl std::_Uninitialized_value_construct_n<class std::allocator<unsigned int>>(unsigned int *, unsigned __int64, class std::allocator<unsigned int> &)")]
	[CleanName("Uninitialized_value_construct_n")]
	public unsafe static void* Uninitialized_value_construct_n_zg4iwva(void* First, long Count, void* Al)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Uninitialized_value_construct_n_zg4iwva.Invoke(First, Count, Al);
	}

	[MangledName("??1?$_Tidy_guard@V?$vector@IV?$allocator@I@std@@@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::_Tidy_guard<class std::vector<unsigned int, class std::allocator<unsigned int>>>::~_Tidy_guard<class std::vector<unsigned int, class std::allocator<unsigned int>>>(void)")]
	public unsafe static void Tidy_guard_class_std_vector_unsigned_int_class_std_allocator_unsigned_int_Destructor(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Tidy_guard_class_std_vector_unsigned_int_class_std_allocator_unsigned_int_Destructor.Invoke(@this);
	}

	[MangledName("?_Release@_Fake_proxy_ptr_impl@std@@QEAAXXZ")]
	[DemangledName("public: void __cdecl std::_Fake_proxy_ptr_impl::_Release(void)")]
	public unsafe static void Release(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Release.Invoke(@this);
	}

	[MangledName("?_Get_first@?$_Compressed_pair@V?$allocator@I@std@@V?$_Vector_val@U?$_Simple_types@I@std@@@2@$00@std@@QEAAAEAV?$allocator@I@2@XZ")]
	[DemangledName("public: class std::allocator<unsigned int> & __cdecl std::_Compressed_pair<class std::allocator<unsigned int>, class std::_Vector_val<struct std::_Simple_types<unsigned int>>, 1>::_Get_first(void)")]
	[CleanName("Get_first")]
	public unsafe static void* Get_first_evhdorb(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_first_evhdorb.Invoke(@this);
	}

	[MangledName("?max_size@?$vector@IV?$allocator@I@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::max_size(void) const")]
	[CleanName("max_size")]
	public unsafe static long max_size_qdwnpha(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.max_size_qdwnpha.Invoke(@this);
	}

	[MangledName("?_Xlength@?$vector@IV?$allocator@I@std@@@std@@CAXXZ")]
	[DemangledName("private: static void __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::_Xlength(void)")]
	[CleanName("Xlength")]
	public static void Xlength_nubzgpc()
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Xlength_nubzgpc.Invoke();
	}

	[MangledName("?_Buy_raw@?$vector@IV?$allocator@I@std@@@std@@AEAAX_K@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::_Buy_raw(unsigned __int64)")]
	public unsafe static void Buy_raw(void* @this, long Newcapacity)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Buy_raw.Invoke(@this, Newcapacity);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??$min@_K@std@@YAAEB_KAEB_K0@Z")]
	[DemangledName("unsigned __int64 const & __cdecl std::min<unsigned __int64>(unsigned __int64 const &, unsigned __int64 const &)")]
	public unsafe static void* min(void* Left, void* Right)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.min.Invoke(Left, Right);
	}

	[MangledName("?max_size@?$_Default_allocator_traits@V?$allocator@I@std@@@std@@SA_KAEBV?$allocator@I@2@@Z")]
	[DemangledName("public: static unsigned __int64 __cdecl std::_Default_allocator_traits<class std::allocator<unsigned int>>::max_size(class std::allocator<unsigned int> const &)")]
	[CleanName("max_size")]
	public unsafe static long max_size_snq294b(void* parameter_0)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.max_size_snq294b.Invoke(parameter_0);
	}

	[MangledName("?_Getal@?$vector@IV?$allocator@I@std@@@std@@AEBAAEBV?$allocator@I@2@XZ")]
	[DemangledName("private: class std::allocator<unsigned int> const & __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::_Getal(void) const")]
	[CleanName("Getal")]
	public unsafe static void* Getal_r5wdlvb(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Getal_r5wdlvb.Invoke(@this);
	}

	[MangledName("??$_Max_limit@_J@std@@YA_JXZ")]
	[DemangledName("__int64 __cdecl std::_Max_limit<__int64>(void)")]
	public static long Max_limit()
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Max_limit.Invoke();
	}

	[MangledName("?_Get_first@?$_Compressed_pair@V?$allocator@I@std@@V?$_Vector_val@U?$_Simple_types@I@std@@@2@$00@std@@QEBAAEBV?$allocator@I@2@XZ")]
	[DemangledName("public: class std::allocator<unsigned int> const & __cdecl std::_Compressed_pair<class std::allocator<unsigned int>, class std::_Vector_val<struct std::_Simple_types<unsigned int>>, 1>::_Get_first(void) const")]
	[CleanName("Get_first")]
	public unsafe static void* Get_first_gs7mvkc(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_first_gs7mvkc.Invoke(@this);
	}

	[MangledName("?_Xlength_error@std@@YAXPEBD@Z")]
	[DemangledName("void __cdecl std::_Xlength_error(char const *)")]
	public unsafe static void Xlength_error(void* parameter_0)
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Unimplemented.Xlength_error.Invoke(parameter_0);
	}

	[MangledName("??$_Allocate_at_least_helper@V?$allocator@I@std@@@std@@YAPEAIAEAV?$allocator@I@0@AEA_K@Z")]
	[DemangledName("unsigned int * __cdecl std::_Allocate_at_least_helper<class std::allocator<unsigned int>>(class std::allocator<unsigned int> &, unsigned __int64 &)")]
	[CleanName("Allocate_at_least_helper")]
	public unsafe static void* Allocate_at_least_helper_22hwyxd(void* Al, void* Count)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.Allocate_at_least_helper_22hwyxd.Invoke(Al, Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?allocate@?$allocator@I@std@@QEAAPEAI_K@Z")]
	[DemangledName("public: unsigned int * __cdecl std::allocator<unsigned int>::allocate(unsigned __int64)")]
	[CleanName("allocate")]
	public unsafe static void* allocate_wor4ptd(void* @this, long Count)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.allocate_wor4ptd.Invoke(@this, Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Allocate@$0BA@U_Default_allocate_traits@std@@@std@@YAPEAX_K@Z")]
	[DemangledName("void * __cdecl std::_Allocate<16, struct std::_Default_allocate_traits>(unsigned __int64)")]
	[CleanName("Allocate")]
	public unsafe static void* Allocate_i4jzs2a(long Bytes)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.Allocate_i4jzs2a.Invoke(Bytes);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Get_size_of_n@$03@std@@YA_K_K@Z")]
	[DemangledName("unsigned __int64 __cdecl std::_Get_size_of_n<4>(unsigned __int64)")]
	[CleanName("Get_size_of_n")]
	public static long Get_size_of_n_rdjmi2b(long Count)
	{
		long result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.Get_size_of_n_rdjmi2b.Invoke(Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Allocate_manually_vector_aligned@U_Default_allocate_traits@std@@@std@@YAPEAX_K@Z")]
	[DemangledName("void * __cdecl std::_Allocate_manually_vector_aligned<struct std::_Default_allocate_traits>(unsigned __int64)")]
	public unsafe static void* Allocate_manually_vector_aligned(long Bytes)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.Allocate_manually_vector_aligned.Invoke(Bytes);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?_Allocate@_Default_allocate_traits@std@@SAPEAX_K@Z")]
	[DemangledName("public: static void * __cdecl std::_Default_allocate_traits::_Allocate(unsigned __int64)")]
	[CleanName("Allocate")]
	public unsafe static void* Allocate_ifu7fia(long Bytes)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Allocate_ifu7fia.Invoke(Bytes);
	}

	[MangledName("?_Throw_bad_array_new_length@std@@YAXXZ")]
	[DemangledName("void __cdecl std::_Throw_bad_array_new_length(void)")]
	public static void Throw_bad_array_new_length()
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Throw_bad_array_new_length.Invoke();
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("llvm.trap")]
	[DemangledName("llvm.trap")]
	public static void llvm_trap()
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Implemented.llvm_trap.Invoke();
	}

	[MangledName("??0bad_array_new_length@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::bad_array_new_length::bad_array_new_length(void)")]
	[CleanName("bad_array_new_length_Constructor")]
	public unsafe static void* bad_array_new_length_Constructor_fnjru5d(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.bad_array_new_length_Constructor_fnjru5d.Invoke(@this);
	}

	[MangledName("??0bad_array_new_length@std@@QEAA@AEBV01@@Z")]
	[DemangledName("public: __cdecl std::bad_array_new_length::bad_array_new_length(class std::bad_array_new_length const &)")]
	[CleanName("bad_array_new_length_Constructor")]
	public unsafe static void* bad_array_new_length_Constructor_7fjuj3c(void* @this, void* parameter_1)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.bad_array_new_length_Constructor_7fjuj3c.Invoke(@this, parameter_1);
	}

	[MangledName("??0bad_alloc@std@@QEAA@AEBV01@@Z")]
	[DemangledName("public: __cdecl std::bad_alloc::bad_alloc(class std::bad_alloc const &)")]
	[CleanName("bad_alloc_Constructor")]
	public unsafe static void* bad_alloc_Constructor_3vdoaxb(void* @this, void* parameter_1)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.bad_alloc_Constructor_3vdoaxb.Invoke(@this, parameter_1);
	}

	[MangledName("??0exception@std@@QEAA@AEBV01@@Z")]
	[DemangledName("public: __cdecl std::exception::exception(class std::exception const &)")]
	[CleanName("exception_Constructor")]
	public unsafe static void* exception_Constructor_ikocrpb(void* @this, void* Other)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.exception_Constructor_ikocrpb.Invoke(@this, Other);
	}

	[MangledName("??1bad_array_new_length@std@@UEAA@XZ")]
	[DemangledName("public: virtual __cdecl std::bad_array_new_length::~bad_array_new_length(void)")]
	public unsafe static void bad_array_new_length_Destructor(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.bad_array_new_length_Destructor.Invoke(@this);
	}

	[MangledName("_CxxThrowException")]
	[DemangledName("_CxxThrowException")]
	public unsafe static void CxxThrowException(void* parameter_0, void* parameter_1)
	{
		try
		{
			AssetRipper.Conversions.FastPng.Intrinsics.Implemented.CxxThrowException.Invoke(parameter_0, parameter_1);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??0bad_alloc@std@@AEAA@QEBD@Z")]
	[DemangledName("private: __cdecl std::bad_alloc::bad_alloc(char const *const)")]
	[CleanName("bad_alloc_Constructor")]
	public unsafe static void* bad_alloc_Constructor_72dj2ea(void* @this, void* Message)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.bad_alloc_Constructor_72dj2ea.Invoke(@this, Message);
	}

	[MangledName("??_Gbad_array_new_length@std@@UEAAPEAXI@Z")]
	[DemangledName("public: virtual void * __cdecl std::bad_array_new_length::`scalar deleting dtor'(unsigned int)")]
	public unsafe static void* bad_array_new_length_Delete(void* @this, int should_call_delete)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.bad_array_new_length_Delete.Invoke(@this, should_call_delete);
	}

	[MangledName("?what@exception@std@@UEBAPEBDXZ")]
	[DemangledName("public: virtual char const * __cdecl std::exception::what(void) const")]
	public unsafe static void* what(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.what.Invoke(@this);
	}

	[MangledName("??0exception@std@@QEAA@QEBDH@Z")]
	[DemangledName("public: __cdecl std::exception::exception(char const *const, int)")]
	[CleanName("exception_Constructor")]
	public unsafe static void* exception_Constructor_morlzmd(void* @this, void* Message, int parameter_2)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.exception_Constructor_morlzmd.Invoke(@this, Message, parameter_2);
	}

	[MangledName("??_Gbad_alloc@std@@UEAAPEAXI@Z")]
	[DemangledName("public: virtual void * __cdecl std::bad_alloc::`scalar deleting dtor'(unsigned int)")]
	public unsafe static void* bad_alloc_Delete(void* @this, int should_call_delete)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.bad_alloc_Delete.Invoke(@this, should_call_delete);
	}

	[MangledName("??_Gexception@std@@UEAAPEAXI@Z")]
	[DemangledName("public: virtual void * __cdecl std::exception::`scalar deleting dtor'(unsigned int)")]
	public unsafe static void* exception_Delete(void* @this, int should_call_delete)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.exception_Delete.Invoke(@this, should_call_delete);
	}

	[MangledName("??1exception@std@@UEAA@XZ")]
	[DemangledName("public: virtual __cdecl std::exception::~exception(void)")]
	public unsafe static void exception_Destructor(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.exception_Destructor.Invoke(@this);
	}

	[MangledName("??3@YAXPEAX_K@Z")]
	[DemangledName("void __cdecl operator delete(void *, unsigned __int64)")]
	public unsafe static void Delete(void* parameter_0, long parameter_1)
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Implemented.Delete.Invoke(parameter_0, parameter_1);
	}

	[MangledName("__std_exception_destroy")]
	[DemangledName("__std_exception_destroy")]
	public unsafe static void std_exception_destroy(void* parameter_0)
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Unimplemented.std_exception_destroy.Invoke(parameter_0);
	}

	[MangledName("__std_terminate")]
	[DemangledName("__std_terminate")]
	public static void std_terminate()
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Implemented.std_terminate.Invoke();
	}

	[MangledName("??1bad_alloc@std@@UEAA@XZ")]
	[DemangledName("public: virtual __cdecl std::bad_alloc::~bad_alloc(void)")]
	public unsafe static void bad_alloc_Destructor(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.bad_alloc_Destructor.Invoke(@this);
	}

	[MangledName("__std_exception_copy")]
	[DemangledName("__std_exception_copy")]
	public unsafe static void std_exception_copy(void* parameter_0, void* parameter_1)
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Unimplemented.std_exception_copy.Invoke(parameter_0, parameter_1);
	}

	[MangledName("??2@YAPEAX_K@Z")]
	[DemangledName("void * __cdecl operator new(unsigned __int64)")]
	public unsafe static void* New(long parameter_0)
	{
		return AssetRipper.Conversions.FastPng.Intrinsics.Implemented.New.Invoke(parameter_0);
	}

	[MangledName("??$_Unfancy@I@std@@YAPEAIPEAI@Z")]
	[DemangledName("unsigned int * __cdecl std::_Unfancy<unsigned int>(unsigned int *)")]
	[CleanName("Unfancy")]
	public unsafe static void* Unfancy_xjzn42c(void* Ptr)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Unfancy_xjzn42c.Invoke(Ptr);
	}

	[MangledName("??$_Zero_range@PEAI@std@@YAPEAIQEAI0@Z")]
	[DemangledName("unsigned int * __cdecl std::_Zero_range<unsigned int *>(unsigned int *const, unsigned int *const)")]
	[CleanName("Zero_range")]
	public unsafe static void* Zero_range_dc39dpa(void* First, void* Last)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Zero_range_dc39dpa.Invoke(First, Last);
	}

	[MangledName("??$_To_address@PEAI@std@@YA?A_PAEBQEAI@Z")]
	[DemangledName("auto __cdecl std::_To_address<unsigned int *>(unsigned int *const &)")]
	[CleanName("To_address")]
	public unsafe static void* To_address_aoz7nfb(void* Val)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.To_address_aoz7nfb.Invoke(Val);
	}

	[MangledName("?_Tidy@?$vector@IV?$allocator@I@std@@@std@@AEAAXXZ")]
	[DemangledName("private: void __cdecl std::vector<unsigned int, class std::allocator<unsigned int>>::_Tidy(void)")]
	[CleanName("Tidy")]
	public unsafe static void Tidy_kyqxybb(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Tidy_kyqxybb.Invoke(@this);
	}

	[MangledName("?_Orphan_all@_Container_base0@std@@QEAAXXZ")]
	[DemangledName("public: void __cdecl std::_Container_base0::_Orphan_all(void)")]
	public unsafe static void Orphan_all(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Orphan_all.Invoke(@this);
	}

	[MangledName("??$_Destroy_range@V?$allocator@I@std@@@std@@YAXPEAIQEAIAEAV?$allocator@I@0@@Z")]
	[DemangledName("void __cdecl std::_Destroy_range<class std::allocator<unsigned int>>(unsigned int *, unsigned int *const, class std::allocator<unsigned int> &)")]
	[CleanName("Destroy_range")]
	public unsafe static void Destroy_range_9rluabc(void* First, void* Last, void* Al)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Destroy_range_9rluabc.Invoke(First, Last, Al);
	}

	[MangledName("?deallocate@?$allocator@I@std@@QEAAXQEAI_K@Z")]
	[DemangledName("public: void __cdecl std::allocator<unsigned int>::deallocate(unsigned int *const, unsigned __int64)")]
	[CleanName("deallocate")]
	public unsafe static void deallocate_gfl4ncc(void* @this, void* Ptr, long Count)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.deallocate_gfl4ncc.Invoke(@this, Ptr, Count);
	}

	[MangledName("??$_Deallocate@$0BA@@std@@YAXPEAX_K@Z")]
	[DemangledName("void __cdecl std::_Deallocate<16>(void *, unsigned __int64)")]
	public unsafe static void Deallocate(void* Ptr, long Bytes)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Deallocate.Invoke(Ptr, Bytes);
	}

	[MangledName("?_Adjust_manually_vector_aligned@std@@YAXAEAPEAXAEA_K@Z")]
	[DemangledName("void __cdecl std::_Adjust_manually_vector_aligned(void *&, unsigned __int64 &)")]
	public unsafe static void Adjust_manually_vector_aligned(void* Ptr, void* Bytes)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Adjust_manually_vector_aligned.Invoke(Ptr, Bytes);
	}

	[MangledName("??$_Unfancy_maybe_null@I@std@@YAPEAIPEAI@Z")]
	[DemangledName("unsigned int * __cdecl std::_Unfancy_maybe_null<unsigned int>(unsigned int *)")]
	[CleanName("Unfancy_maybe_null")]
	public unsafe static void* Unfancy_maybe_null_ys5czvb(void* Ptr)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Unfancy_maybe_null_ys5czvb.Invoke(Ptr);
	}

	[MangledName("??$maximum@I@fpng@@YAIII@Z")]
	[DemangledName("unsigned int __cdecl fpng::maximum<unsigned int>(unsigned int, unsigned int)")]
	[CleanName("maximum")]
	public static int maximum_y5wzmwb(int a, int b)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.maximum_y5wzmwb.Invoke(a, b);
	}

	[MangledName("?defl_optimize_huffman_table@fpng@@YAXPEAUdefl_huff@1@HHHH@Z")]
	[DemangledName("void __cdecl fpng::defl_optimize_huffman_table(struct fpng::defl_huff *, int, int, int, int)")]
	public unsafe static void defl_optimize_huffman_table(void* d, int table_num, int table_len, int code_size_limit, int static_table)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.defl_optimize_huffman_table.Invoke(d, table_num, table_len, code_size_limit, static_table);
	}

	[MangledName("??$maximum@H@fpng@@YAHHH@Z")]
	[DemangledName("int __cdecl fpng::maximum<int>(int, int)")]
	[CleanName("maximum")]
	public static int maximum_sfjssib(int a, int b)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.maximum_sfjssib.Invoke(a, b);
	}

	[MangledName("?defl_radix_sort_syms@fpng@@YAPEAUdefl_sym_freq@1@IPEAU21@0@Z")]
	[DemangledName("struct fpng::defl_sym_freq * __cdecl fpng::defl_radix_sort_syms(unsigned int, struct fpng::defl_sym_freq *, struct fpng::defl_sym_freq *)")]
	public unsafe static void* defl_radix_sort_syms(int num_syms, void* pSyms0, void* pSyms1)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.defl_radix_sort_syms.Invoke(num_syms, pSyms0, pSyms1);
	}

	[MangledName("?defl_calculate_minimum_redundancy@fpng@@YAXPEAUdefl_sym_freq@1@H@Z")]
	[DemangledName("void __cdecl fpng::defl_calculate_minimum_redundancy(struct fpng::defl_sym_freq *, int)")]
	public unsafe static void defl_calculate_minimum_redundancy(void* A, int n)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.defl_calculate_minimum_redundancy.Invoke(A, n);
	}

	[MangledName("?defl_huffman_enforce_max_code_size@fpng@@YAXPEAHHH@Z")]
	[DemangledName("void __cdecl fpng::defl_huffman_enforce_max_code_size(int *, int, int)")]
	public unsafe static void defl_huffman_enforce_max_code_size(void* pNum_codes, int code_list_len, int max_code_size)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.defl_huffman_enforce_max_code_size.Invoke(pNum_codes, code_list_len, max_code_size);
	}

	[MangledName("??0?$vector@_KV?$allocator@_K@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::vector<unsigned __int64, class std::allocator<unsigned __int64>>(void)")]
	public unsafe static void* vector_unsigned_int64_class_std_allocator_unsigned_int64_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_int64_class_std_allocator_unsigned_int64_Constructor.Invoke(@this);
	}

	[MangledName("?resize@?$vector@_KV?$allocator@_K@std@@@std@@QEAAX_K@Z")]
	[DemangledName("public: void __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::resize(unsigned __int64)")]
	[CleanName("resize")]
	public unsafe static void resize_d4pmvud(void* @this, long Newsize)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.resize_d4pmvud.Invoke(@this, Newsize);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("?data@?$vector@_KV?$allocator@_K@std@@@std@@QEAAPEA_KXZ")]
	[DemangledName("public: unsigned __int64 * __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::data(void)")]
	[CleanName("data")]
	public unsafe static void* data_dtgl25a(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.data_dtgl25a.Invoke(@this);
	}

	[MangledName("?size@?$vector@_KV?$allocator@_K@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::size(void) const")]
	[CleanName("size")]
	public unsafe static long size_trt5i3d(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.size_trt5i3d.Invoke(@this);
	}

	[MangledName("??A?$vector@_KV?$allocator@_K@std@@@std@@QEAAAEA_K_K@Z")]
	[DemangledName("public: unsigned __int64 & __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::operator[](unsigned __int64)")]
	public unsafe static void* vector_unsigned_int64_class_std_allocator_unsigned_int64_Index(void* @this, long Pos)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_int64_class_std_allocator_unsigned_int64_Index.Invoke(@this, Pos);
	}

	[MangledName("??1?$vector@_KV?$allocator@_K@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::~vector<unsigned __int64, class std::allocator<unsigned __int64>>(void)")]
	public unsafe static void vector_unsigned_int64_class_std_allocator_unsigned_int64_Destructor(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.vector_unsigned_int64_class_std_allocator_unsigned_int64_Destructor.Invoke(@this);
	}

	[MangledName("??$?0$$V@?$_Compressed_pair@V?$allocator@_K@std@@V?$_Vector_val@U?$_Simple_types@_K@std@@@2@$00@std@@QEAA@U_Zero_then_variadic_args_t@1@@Z")]
	[DemangledName("public: __cdecl std::_Compressed_pair<class std::allocator<unsigned __int64>, class std::_Vector_val<struct std::_Simple_types<unsigned __int64>>, 1>::_Compressed_pair<class std::allocator<unsigned __int64>, class std::_Vector_val<struct std::_Simple_types<unsigned __int64>>, 1><>(struct std::_Zero_then_variadic_args_t)")]
	public unsafe static void* Compressed_pair_class_std_allocator_unsigned_int64_class_std_Vector_val_struct_std_Simple_types_unsigned_int64_1_Constructor(void* @this, sbyte coerce)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Compressed_pair_class_std_allocator_unsigned_int64_class_std_Vector_val_struct_std_Simple_types_unsigned_int64_1_Constructor.Invoke(@this, coerce);
	}

	[MangledName("?_Alloc_proxy@_Container_base0@std@@QEAAXAEBU_Fake_allocator@2@@Z")]
	[DemangledName("public: void __cdecl std::_Container_base0::_Alloc_proxy(struct std::_Fake_allocator const &)")]
	public unsafe static void Alloc_proxy(void* @this, void* parameter_1)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Alloc_proxy.Invoke(@this, parameter_1);
	}

	[MangledName("??$_Get_proxy_allocator@V?$allocator@_K@std@@@std@@YA?AU_Fake_allocator@0@AEBV?$allocator@_K@0@@Z")]
	[DemangledName("struct std::_Fake_allocator __cdecl std::_Get_proxy_allocator<class std::allocator<unsigned __int64>>(class std::allocator<unsigned __int64> const &)")]
	[CleanName("Get_proxy_allocator")]
	public unsafe static sbyte Get_proxy_allocator_9e3ilvb(void* parameter_0)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_proxy_allocator_9e3ilvb.Invoke(parameter_0);
	}

	[MangledName("?_Getal@?$vector@_KV?$allocator@_K@std@@@std@@AEAAAEAV?$allocator@_K@2@XZ")]
	[DemangledName("private: class std::allocator<unsigned __int64> & __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Getal(void)")]
	[CleanName("Getal")]
	public unsafe static void* Getal_kuj7nva(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Getal_kuj7nva.Invoke(@this);
	}

	[MangledName("??0?$allocator@_K@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::allocator<unsigned __int64>::allocator<unsigned __int64>(void)")]
	public unsafe static void* allocator_unsigned_int64_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.allocator_unsigned_int64_Constructor.Invoke(@this);
	}

	[MangledName("??0?$_Vector_val@U?$_Simple_types@_K@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::_Vector_val<struct std::_Simple_types<unsigned __int64>>::_Vector_val<struct std::_Simple_types<unsigned __int64>>(void)")]
	public unsafe static void* Vector_val_struct_std_Simple_types_unsigned_int64_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Vector_val_struct_std_Simple_types_unsigned_int64_Constructor.Invoke(@this);
	}

	[MangledName("?_Get_first@?$_Compressed_pair@V?$allocator@_K@std@@V?$_Vector_val@U?$_Simple_types@_K@std@@@2@$00@std@@QEAAAEAV?$allocator@_K@2@XZ")]
	[DemangledName("public: class std::allocator<unsigned __int64> & __cdecl std::_Compressed_pair<class std::allocator<unsigned __int64>, class std::_Vector_val<struct std::_Simple_types<unsigned __int64>>, 1>::_Get_first(void)")]
	[CleanName("Get_first")]
	public unsafe static void* Get_first_whlbtqb(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_first_whlbtqb.Invoke(@this);
	}

	[MangledName("??$_Resize@U_Value_init_tag@std@@@?$vector@_KV?$allocator@_K@std@@@std@@AEAAX_KAEBU_Value_init_tag@1@@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Resize<struct std::_Value_init_tag>(unsigned __int64, struct std::_Value_init_tag const &)")]
	[CleanName("Resize")]
	public unsafe static void Resize_2s9rs9d(void* @this, long Newsize, void* Val)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Resize_2s9rs9d.Invoke(@this, Newsize, Val);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("?_Orphan_range@?$vector@_KV?$allocator@_K@std@@@std@@AEBAXPEA_K0@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Orphan_range(unsigned __int64 *, unsigned __int64 *) const")]
	[CleanName("Orphan_range")]
	public unsafe static void Orphan_range_qciws4b(void* @this, void* parameter_1, void* parameter_2)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Orphan_range_qciws4b.Invoke(@this, parameter_1, parameter_2);
	}

	[MangledName("??$_Destroy_range@V?$allocator@_K@std@@@std@@YAXPEA_KQEA_KAEAV?$allocator@_K@0@@Z")]
	[DemangledName("void __cdecl std::_Destroy_range<class std::allocator<unsigned __int64>>(unsigned __int64 *, unsigned __int64 *const, class std::allocator<unsigned __int64> &)")]
	[CleanName("Destroy_range")]
	public unsafe static void Destroy_range_qbzp7xb(void* First, void* Last, void* Al)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Destroy_range_qbzp7xb.Invoke(First, Last, Al);
	}

	[MangledName("??$_Resize_reallocate@U_Value_init_tag@std@@@?$vector@_KV?$allocator@_K@std@@@std@@AEAAX_KAEBU_Value_init_tag@1@@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Resize_reallocate<struct std::_Value_init_tag>(unsigned __int64, struct std::_Value_init_tag const &)")]
	[CleanName("Resize_reallocate")]
	public unsafe static void Resize_reallocate_ytnsorc(void* @this, long Newsize, void* Val)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Resize_reallocate_ytnsorc.Invoke(@this, Newsize, Val);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??$_Uninitialized_value_construct_n@V?$allocator@_K@std@@@std@@YAPEA_KPEA_K_KAEAV?$allocator@_K@0@@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Uninitialized_value_construct_n<class std::allocator<unsigned __int64>>(unsigned __int64 *, unsigned __int64, class std::allocator<unsigned __int64> &)")]
	[CleanName("Uninitialized_value_construct_n")]
	public unsafe static void* Uninitialized_value_construct_n_hqowyoc(void* First, long Count, void* Al)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Uninitialized_value_construct_n_hqowyoc.Invoke(First, Count, Al);
	}

	[MangledName("?max_size@?$vector@_KV?$allocator@_K@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::max_size(void) const")]
	[CleanName("max_size")]
	public unsafe static long max_size_kyvxnnb(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.max_size_kyvxnnb.Invoke(@this);
	}

	[MangledName("?_Xlength@?$vector@_KV?$allocator@_K@std@@@std@@CAXXZ")]
	[DemangledName("private: static void __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Xlength(void)")]
	[CleanName("Xlength")]
	public static void Xlength_zmqdsxa()
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Xlength_zmqdsxa.Invoke();
	}

	[MangledName("?_Calculate_growth@?$vector@_KV?$allocator@_K@std@@@std@@AEBA_K_K@Z")]
	[DemangledName("private: unsigned __int64 __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Calculate_growth(unsigned __int64) const")]
	[CleanName("Calculate_growth")]
	public unsafe static long Calculate_growth_vbhwmec(void* @this, long Newsize)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Calculate_growth_vbhwmec.Invoke(@this, Newsize);
	}

	[MangledName("??$_Allocate_at_least_helper@V?$allocator@_K@std@@@std@@YAPEA_KAEAV?$allocator@_K@0@AEA_K@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Allocate_at_least_helper<class std::allocator<unsigned __int64>>(class std::allocator<unsigned __int64> &, unsigned __int64 &)")]
	[CleanName("Allocate_at_least_helper")]
	public unsafe static void* Allocate_at_least_helper_l9zi7qb(void* Al, void* Count)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.Allocate_at_least_helper_l9zi7qb.Invoke(Al, Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Uninitialized_move@PEA_KV?$allocator@_K@std@@@std@@YAPEA_KQEA_K0PEA_KAEAV?$allocator@_K@0@@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Uninitialized_move<unsigned __int64 *, class std::allocator<unsigned __int64>>(unsigned __int64 *const, unsigned __int64 *const, unsigned __int64 *, class std::allocator<unsigned __int64> &)")]
	[CleanName("Uninitialized_move")]
	public unsafe static void* Uninitialized_move_cbc4pbc(void* First, void* Last, void* Dest, void* Al)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Uninitialized_move_cbc4pbc.Invoke(First, Last, Dest, Al);
	}

	[MangledName("?_Change_array@?$vector@_KV?$allocator@_K@std@@@std@@AEAAXQEA_K_K1@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Change_array(unsigned __int64 *const, unsigned __int64, unsigned __int64)")]
	[CleanName("Change_array")]
	public unsafe static void Change_array_hgqy2vc(void* @this, void* Newvec, long Newsize, long Newcapacity)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Change_array_hgqy2vc.Invoke(@this, Newvec, Newsize, Newcapacity);
	}

	[MangledName("??1_Reallocation_guard@?$vector@_KV?$allocator@_K@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Reallocation_guard::~_Reallocation_guard(void)")]
	[CleanName("Reallocation_guard_Destructor")]
	public unsafe static void Reallocation_guard_Destructor_jfaqped(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Reallocation_guard_Destructor_jfaqped.Invoke(@this);
	}

	[MangledName("?max_size@?$_Default_allocator_traits@V?$allocator@_K@std@@@std@@SA_KAEBV?$allocator@_K@2@@Z")]
	[DemangledName("public: static unsigned __int64 __cdecl std::_Default_allocator_traits<class std::allocator<unsigned __int64>>::max_size(class std::allocator<unsigned __int64> const &)")]
	[CleanName("max_size")]
	public unsafe static long max_size_ngjedfc(void* parameter_0)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.max_size_ngjedfc.Invoke(parameter_0);
	}

	[MangledName("?_Getal@?$vector@_KV?$allocator@_K@std@@@std@@AEBAAEBV?$allocator@_K@2@XZ")]
	[DemangledName("private: class std::allocator<unsigned __int64> const & __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Getal(void) const")]
	[CleanName("Getal")]
	public unsafe static void* Getal_2reycsb(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Getal_2reycsb.Invoke(@this);
	}

	[MangledName("?_Get_first@?$_Compressed_pair@V?$allocator@_K@std@@V?$_Vector_val@U?$_Simple_types@_K@std@@@2@$00@std@@QEBAAEBV?$allocator@_K@2@XZ")]
	[DemangledName("public: class std::allocator<unsigned __int64> const & __cdecl std::_Compressed_pair<class std::allocator<unsigned __int64>, class std::_Vector_val<struct std::_Simple_types<unsigned __int64>>, 1>::_Get_first(void) const")]
	[CleanName("Get_first")]
	public unsafe static void* Get_first_gcgh4xa(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_first_gcgh4xa.Invoke(@this);
	}

	[MangledName("?capacity@?$vector@_KV?$allocator@_K@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::capacity(void) const")]
	[CleanName("capacity")]
	public unsafe static long capacity_f49yk4c(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.capacity_f49yk4c.Invoke(@this);
	}

	[MangledName("?allocate@?$allocator@_K@std@@QEAAPEA_K_K@Z")]
	[DemangledName("public: unsigned __int64 * __cdecl std::allocator<unsigned __int64>::allocate(unsigned __int64)")]
	[CleanName("allocate")]
	public unsafe static void* allocate_4vpkydd(void* @this, long Count)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.allocate_4vpkydd.Invoke(@this, Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Get_size_of_n@$07@std@@YA_K_K@Z")]
	[DemangledName("unsigned __int64 __cdecl std::_Get_size_of_n<8>(unsigned __int64)")]
	[CleanName("Get_size_of_n")]
	public static long Get_size_of_n_siqup9d(long Count)
	{
		long result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.Get_size_of_n_siqup9d.Invoke(Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Get_unwrapped@AEBQEA_K@std@@YA?A_TAEBQEA_K@Z")]
	[DemangledName("decltype(auto) __cdecl std::_Get_unwrapped<unsigned __int64 *const &>(unsigned __int64 *const &)")]
	[CleanName("Get_unwrapped")]
	public unsafe static void* Get_unwrapped_3jsimya(void* It)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_unwrapped_3jsimya.Invoke(It);
	}

	[MangledName("??$_Copy_memmove@PEA_KPEA_K@std@@YAPEA_KPEA_K00@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Copy_memmove<unsigned __int64 *, unsigned __int64 *>(unsigned __int64 *, unsigned __int64 *, unsigned __int64 *)")]
	[CleanName("Copy_memmove")]
	public unsafe static void* Copy_memmove_uofrjzc(void* First, void* Last, void* Dest)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Copy_memmove_uofrjzc.Invoke(First, Last, Dest);
	}

	[MangledName("??$_Unfancy@_K@std@@YAPEA_KPEA_K@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Unfancy<unsigned __int64>(unsigned __int64 *)")]
	[CleanName("Unfancy")]
	public unsafe static void* Unfancy_z2hkbxa(void* Ptr)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Unfancy_z2hkbxa.Invoke(Ptr);
	}

	[MangledName("??$_To_address@PEA_K@std@@YA?A_PAEBQEA_K@Z")]
	[DemangledName("auto __cdecl std::_To_address<unsigned __int64 *>(unsigned __int64 *const &)")]
	[CleanName("To_address")]
	public unsafe static void* To_address_l9253rd(void* Val)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.To_address_l9253rd.Invoke(Val);
	}

	[MangledName("??$_Copy_memmove_tail@PEA_K@std@@YAPEA_KQEBDQEA_K_K2@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Copy_memmove_tail<unsigned __int64 *>(char const *const, unsigned __int64 *const, unsigned __int64, unsigned __int64)")]
	[CleanName("Copy_memmove_tail")]
	public unsafe static void* Copy_memmove_tail_oluaukc(void* First_ch, void* Dest, long Byte_count, long Object_count)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Copy_memmove_tail_oluaukc.Invoke(First_ch, Dest, Byte_count, Object_count);
	}

	[MangledName("llvm.memmove.p0.p0.i64")]
	[DemangledName("llvm.memmove.p0.p0.i64")]
	public unsafe static void llvm_memmove_p0_p0_i64(void* parameter_0, void* parameter_1, long parameter_2, bool parameter_3)
	{
		AssetRipper.Conversions.FastPng.Intrinsics.Implemented.llvm_memmove_p0_p0_i64.Invoke(parameter_0, parameter_1, parameter_2, parameter_3);
	}

	[MangledName("?deallocate@?$allocator@_K@std@@QEAAXQEA_K_K@Z")]
	[DemangledName("public: void __cdecl std::allocator<unsigned __int64>::deallocate(unsigned __int64 *const, unsigned __int64)")]
	[CleanName("deallocate")]
	public unsafe static void deallocate_yb2gvqa(void* @this, void* Ptr, long Count)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.deallocate_yb2gvqa.Invoke(@this, Ptr, Count);
	}

	[MangledName("??$_Zero_range@PEA_K@std@@YAPEA_KQEA_K0@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Zero_range<unsigned __int64 *>(unsigned __int64 *const, unsigned __int64 *const)")]
	[CleanName("Zero_range")]
	public unsafe static void* Zero_range_lx9svca(void* First, void* Last)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Zero_range_lx9svca.Invoke(First, Last);
	}

	[MangledName("??$_Unfancy_maybe_null@_K@std@@YAPEA_KPEA_K@Z")]
	[DemangledName("unsigned __int64 * __cdecl std::_Unfancy_maybe_null<unsigned __int64>(unsigned __int64 *)")]
	[CleanName("Unfancy_maybe_null")]
	public unsafe static void* Unfancy_maybe_null_uqd3pbd(void* Ptr)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Unfancy_maybe_null_uqd3pbd.Invoke(Ptr);
	}

	[MangledName("?_Tidy@?$vector@_KV?$allocator@_K@std@@@std@@AEAAXXZ")]
	[DemangledName("private: void __cdecl std::vector<unsigned __int64, class std::allocator<unsigned __int64>>::_Tidy(void)")]
	[CleanName("Tidy")]
	public unsafe static void Tidy_hq3jjxb(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Tidy_hq3jjxb.Invoke(@this);
	}

	[MangledName("??$minimum@I@fpng@@YAIII@Z")]
	[DemangledName("unsigned int __cdecl fpng::minimum<unsigned int>(unsigned int, unsigned int)")]
	[CleanName("minimum")]
	public static int minimum_su2nfub(int a, int b)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.minimum_su2nfub.Invoke(a, b);
	}

	public unsafe static int memcmp(void* parameter_0, void* parameter_1, long parameter_2)
	{
		return AssetRipper.Conversions.FastPng.Intrinsics.Implemented.memcmp.Invoke(parameter_0, parameter_1, parameter_2);
	}

	[MangledName("?swap32@fpng@@YAII@Z")]
	[DemangledName("unsigned int __cdecl fpng::swap32(unsigned int)")]
	public static int swap32(int x)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.swap32.Invoke(x);
	}

	public unsafe static int strcmp(void* parameter_0, void* parameter_1)
	{
		return AssetRipper.Conversions.FastPng.Intrinsics.Implemented.strcmp.Invoke(parameter_0, parameter_1);
	}

	[MangledName("llvm.bswap.i32")]
	[DemangledName("llvm.bswap.i32")]
	public static int llvm_bswap_i32(int parameter_0)
	{
		return AssetRipper.Conversions.FastPng.Intrinsics.Implemented.llvm_bswap_i32.Invoke(parameter_0);
	}

	[MangledName("??$_Resize@U_Value_init_tag@std@@@?$vector@EV?$allocator@E@std@@@std@@AEAAX_KAEBU_Value_init_tag@1@@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Resize<struct std::_Value_init_tag>(unsigned __int64, struct std::_Value_init_tag const &)")]
	[CleanName("Resize")]
	public unsafe static void Resize_p4x2rua(void* @this, long Newsize, void* Val)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Resize_p4x2rua.Invoke(@this, Newsize, Val);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("?_Getal@?$vector@EV?$allocator@E@std@@@std@@AEAAAEAV?$allocator@E@2@XZ")]
	[DemangledName("private: class std::allocator<unsigned char> & __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Getal(void)")]
	[CleanName("Getal")]
	public unsafe static void* Getal_jms5hzb(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Getal_jms5hzb.Invoke(@this);
	}

	[MangledName("?_Orphan_range@?$vector@EV?$allocator@E@std@@@std@@AEBAXPEAE0@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Orphan_range(unsigned char *, unsigned char *) const")]
	[CleanName("Orphan_range")]
	public unsafe static void Orphan_range_334quvb(void* @this, void* parameter_1, void* parameter_2)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Orphan_range_334quvb.Invoke(@this, parameter_1, parameter_2);
	}

	[MangledName("??$_Destroy_range@V?$allocator@E@std@@@std@@YAXPEAEQEAEAEAV?$allocator@E@0@@Z")]
	[DemangledName("void __cdecl std::_Destroy_range<class std::allocator<unsigned char>>(unsigned char *, unsigned char *const, class std::allocator<unsigned char> &)")]
	[CleanName("Destroy_range")]
	public unsafe static void Destroy_range_jjcob5a(void* First, void* Last, void* Al)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Destroy_range_jjcob5a.Invoke(First, Last, Al);
	}

	[MangledName("??$_Resize_reallocate@U_Value_init_tag@std@@@?$vector@EV?$allocator@E@std@@@std@@AEAAX_KAEBU_Value_init_tag@1@@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Resize_reallocate<struct std::_Value_init_tag>(unsigned __int64, struct std::_Value_init_tag const &)")]
	[CleanName("Resize_reallocate")]
	public unsafe static void Resize_reallocate_dt5hjdb(void* @this, long Newsize, void* Val)
	{
		try
		{
			AssetRipper.Conversions.FastPng.GlobalFunctions.Resize_reallocate_dt5hjdb.Invoke(@this, Newsize, Val);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
	}

	[MangledName("??$_Uninitialized_value_construct_n@V?$allocator@E@std@@@std@@YAPEAEPEAE_KAEAV?$allocator@E@0@@Z")]
	[DemangledName("unsigned char * __cdecl std::_Uninitialized_value_construct_n<class std::allocator<unsigned char>>(unsigned char *, unsigned __int64, class std::allocator<unsigned char> &)")]
	[CleanName("Uninitialized_value_construct_n")]
	public unsafe static void* Uninitialized_value_construct_n_cxm3ryd(void* First, long Count, void* Al)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Uninitialized_value_construct_n_cxm3ryd.Invoke(First, Count, Al);
	}

	[MangledName("?_Get_first@?$_Compressed_pair@V?$allocator@E@std@@V?$_Vector_val@U?$_Simple_types@E@std@@@2@$00@std@@QEAAAEAV?$allocator@E@2@XZ")]
	[DemangledName("public: class std::allocator<unsigned char> & __cdecl std::_Compressed_pair<class std::allocator<unsigned char>, class std::_Vector_val<struct std::_Simple_types<unsigned char>>, 1>::_Get_first(void)")]
	[CleanName("Get_first")]
	public unsafe static void* Get_first_nkl7psd(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_first_nkl7psd.Invoke(@this);
	}

	[MangledName("?max_size@?$vector@EV?$allocator@E@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::max_size(void) const")]
	[CleanName("max_size")]
	public unsafe static long max_size_2yjes5b(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.max_size_2yjes5b.Invoke(@this);
	}

	[MangledName("?_Xlength@?$vector@EV?$allocator@E@std@@@std@@CAXXZ")]
	[DemangledName("private: static void __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Xlength(void)")]
	[CleanName("Xlength")]
	public static void Xlength_zpuxaia()
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Xlength_zpuxaia.Invoke();
	}

	[MangledName("?_Calculate_growth@?$vector@EV?$allocator@E@std@@@std@@AEBA_K_K@Z")]
	[DemangledName("private: unsigned __int64 __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Calculate_growth(unsigned __int64) const")]
	[CleanName("Calculate_growth")]
	public unsafe static long Calculate_growth_scmxisc(void* @this, long Newsize)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Calculate_growth_scmxisc.Invoke(@this, Newsize);
	}

	[MangledName("??$_Allocate_at_least_helper@V?$allocator@E@std@@@std@@YAPEAEAEAV?$allocator@E@0@AEA_K@Z")]
	[DemangledName("unsigned char * __cdecl std::_Allocate_at_least_helper<class std::allocator<unsigned char>>(class std::allocator<unsigned char> &, unsigned __int64 &)")]
	[CleanName("Allocate_at_least_helper")]
	public unsafe static void* Allocate_at_least_helper_wikabbc(void* Al, void* Count)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.Allocate_at_least_helper_wikabbc.Invoke(Al, Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Uninitialized_move@PEAEV?$allocator@E@std@@@std@@YAPEAEQEAE0PEAEAEAV?$allocator@E@0@@Z")]
	[DemangledName("unsigned char * __cdecl std::_Uninitialized_move<unsigned char *, class std::allocator<unsigned char>>(unsigned char *const, unsigned char *const, unsigned char *, class std::allocator<unsigned char> &)")]
	[CleanName("Uninitialized_move")]
	public unsafe static void* Uninitialized_move_vysyj4a(void* First, void* Last, void* Dest, void* Al)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Uninitialized_move_vysyj4a.Invoke(First, Last, Dest, Al);
	}

	[MangledName("?_Change_array@?$vector@EV?$allocator@E@std@@@std@@AEAAXQEAE_K1@Z")]
	[DemangledName("private: void __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Change_array(unsigned char *const, unsigned __int64, unsigned __int64)")]
	[CleanName("Change_array")]
	public unsafe static void Change_array_k9lpzqa(void* @this, void* Newvec, long Newsize, long Newcapacity)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Change_array_k9lpzqa.Invoke(@this, Newvec, Newsize, Newcapacity);
	}

	[MangledName("??1_Reallocation_guard@?$vector@EV?$allocator@E@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Reallocation_guard::~_Reallocation_guard(void)")]
	[CleanName("Reallocation_guard_Destructor")]
	public unsafe static void Reallocation_guard_Destructor_9xb7gra(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Reallocation_guard_Destructor_9xb7gra.Invoke(@this);
	}

	[MangledName("?max_size@?$_Default_allocator_traits@V?$allocator@E@std@@@std@@SA_KAEBV?$allocator@E@2@@Z")]
	[DemangledName("public: static unsigned __int64 __cdecl std::_Default_allocator_traits<class std::allocator<unsigned char>>::max_size(class std::allocator<unsigned char> const &)")]
	[CleanName("max_size")]
	public unsafe static long max_size_hv9dl3d(void* parameter_0)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.max_size_hv9dl3d.Invoke(parameter_0);
	}

	[MangledName("?_Getal@?$vector@EV?$allocator@E@std@@@std@@AEBAAEBV?$allocator@E@2@XZ")]
	[DemangledName("private: class std::allocator<unsigned char> const & __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Getal(void) const")]
	[CleanName("Getal")]
	public unsafe static void* Getal_llls4cc(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Getal_llls4cc.Invoke(@this);
	}

	[MangledName("?_Get_first@?$_Compressed_pair@V?$allocator@E@std@@V?$_Vector_val@U?$_Simple_types@E@std@@@2@$00@std@@QEBAAEBV?$allocator@E@2@XZ")]
	[DemangledName("public: class std::allocator<unsigned char> const & __cdecl std::_Compressed_pair<class std::allocator<unsigned char>, class std::_Vector_val<struct std::_Simple_types<unsigned char>>, 1>::_Get_first(void) const")]
	[CleanName("Get_first")]
	public unsafe static void* Get_first_pnsruja(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_first_pnsruja.Invoke(@this);
	}

	[MangledName("?capacity@?$vector@EV?$allocator@E@std@@@std@@QEBA_KXZ")]
	[DemangledName("public: unsigned __int64 __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::capacity(void) const")]
	[CleanName("capacity")]
	public unsafe static long capacity_mz5ysja(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.capacity_mz5ysja.Invoke(@this);
	}

	[MangledName("?allocate@?$allocator@E@std@@QEAAPEAE_K@Z")]
	[DemangledName("public: unsigned char * __cdecl std::allocator<unsigned char>::allocate(unsigned __int64)")]
	[CleanName("allocate")]
	public unsafe static void* allocate_qfop4bd(void* @this, long Count)
	{
		void* result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.allocate_qfop4bd.Invoke(@this, Count);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("??$_Get_size_of_n@$00@std@@YA_K_K@Z")]
	[DemangledName("unsigned __int64 __cdecl std::_Get_size_of_n<1>(unsigned __int64)")]
	[CleanName("Get_size_of_n")]
	public static long Get_size_of_n_iqtw9sb(long Count)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_size_of_n_iqtw9sb.Invoke(Count);
	}

	[MangledName("??$_Get_unwrapped@AEBQEAE@std@@YA?A_TAEBQEAE@Z")]
	[DemangledName("decltype(auto) __cdecl std::_Get_unwrapped<unsigned char *const &>(unsigned char *const &)")]
	[CleanName("Get_unwrapped")]
	public unsafe static void* Get_unwrapped_z99fpma(void* It)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_unwrapped_z99fpma.Invoke(It);
	}

	[MangledName("??$_Copy_memmove@PEAEPEAE@std@@YAPEAEPEAE00@Z")]
	[DemangledName("unsigned char * __cdecl std::_Copy_memmove<unsigned char *, unsigned char *>(unsigned char *, unsigned char *, unsigned char *)")]
	[CleanName("Copy_memmove")]
	public unsafe static void* Copy_memmove_2kkdavd(void* First, void* Last, void* Dest)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Copy_memmove_2kkdavd.Invoke(First, Last, Dest);
	}

	[MangledName("??$_Unfancy@E@std@@YAPEAEPEAE@Z")]
	[DemangledName("unsigned char * __cdecl std::_Unfancy<unsigned char>(unsigned char *)")]
	[CleanName("Unfancy")]
	public unsafe static void* Unfancy_aszk3nc(void* Ptr)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Unfancy_aszk3nc.Invoke(Ptr);
	}

	[MangledName("??$_To_address@PEAE@std@@YA?A_PAEBQEAE@Z")]
	[DemangledName("auto __cdecl std::_To_address<unsigned char *>(unsigned char *const &)")]
	[CleanName("To_address")]
	public unsafe static void* To_address_enkoctc(void* Val)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.To_address_enkoctc.Invoke(Val);
	}

	[MangledName("??$_Copy_memmove_tail@PEAE@std@@YAPEAEQEBDQEAE_K2@Z")]
	[DemangledName("unsigned char * __cdecl std::_Copy_memmove_tail<unsigned char *>(char const *const, unsigned char *const, unsigned __int64, unsigned __int64)")]
	[CleanName("Copy_memmove_tail")]
	public unsafe static void* Copy_memmove_tail_x7ipzed(void* First_ch, void* Dest, long Byte_count, long Object_count)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Copy_memmove_tail_x7ipzed.Invoke(First_ch, Dest, Byte_count, Object_count);
	}

	[MangledName("?deallocate@?$allocator@E@std@@QEAAXQEAE_K@Z")]
	[DemangledName("public: void __cdecl std::allocator<unsigned char>::deallocate(unsigned char *const, unsigned __int64)")]
	[CleanName("deallocate")]
	public unsafe static void deallocate_mc9rbjc(void* @this, void* Ptr, long Count)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.deallocate_mc9rbjc.Invoke(@this, Ptr, Count);
	}

	[MangledName("??$_Zero_range@PEAE@std@@YAPEAEQEAE0@Z")]
	[DemangledName("unsigned char * __cdecl std::_Zero_range<unsigned char *>(unsigned char *const, unsigned char *const)")]
	[CleanName("Zero_range")]
	public unsafe static void* Zero_range_927ahid(void* First, void* Last)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Zero_range_927ahid.Invoke(First, Last);
	}

	[MangledName("??$_Unfancy_maybe_null@E@std@@YAPEAEPEAE@Z")]
	[DemangledName("unsigned char * __cdecl std::_Unfancy_maybe_null<unsigned char>(unsigned char *)")]
	[CleanName("Unfancy_maybe_null")]
	public unsafe static void* Unfancy_maybe_null_pj5f7cb(void* Ptr)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Unfancy_maybe_null_pj5f7cb.Invoke(Ptr);
	}

	[MangledName("??$?0$$V@?$_Compressed_pair@V?$allocator@E@std@@V?$_Vector_val@U?$_Simple_types@E@std@@@2@$00@std@@QEAA@U_Zero_then_variadic_args_t@1@@Z")]
	[DemangledName("public: __cdecl std::_Compressed_pair<class std::allocator<unsigned char>, class std::_Vector_val<struct std::_Simple_types<unsigned char>>, 1>::_Compressed_pair<class std::allocator<unsigned char>, class std::_Vector_val<struct std::_Simple_types<unsigned char>>, 1><>(struct std::_Zero_then_variadic_args_t)")]
	public unsafe static void* Compressed_pair_class_std_allocator_unsigned_char_class_std_Vector_val_struct_std_Simple_types_unsigned_char_1_Constructor(void* @this, sbyte coerce)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Compressed_pair_class_std_allocator_unsigned_char_class_std_Vector_val_struct_std_Simple_types_unsigned_char_1_Constructor.Invoke(@this, coerce);
	}

	[MangledName("??$_Get_proxy_allocator@V?$allocator@E@std@@@std@@YA?AU_Fake_allocator@0@AEBV?$allocator@E@0@@Z")]
	[DemangledName("struct std::_Fake_allocator __cdecl std::_Get_proxy_allocator<class std::allocator<unsigned char>>(class std::allocator<unsigned char> const &)")]
	[CleanName("Get_proxy_allocator")]
	public unsafe static sbyte Get_proxy_allocator_zrfshta(void* parameter_0)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Get_proxy_allocator_zrfshta.Invoke(parameter_0);
	}

	[MangledName("??0?$allocator@E@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::allocator<unsigned char>::allocator<unsigned char>(void)")]
	public unsafe static void* allocator_unsigned_char_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.allocator_unsigned_char_Constructor.Invoke(@this);
	}

	[MangledName("??0?$_Vector_val@U?$_Simple_types@E@std@@@std@@QEAA@XZ")]
	[DemangledName("public: __cdecl std::_Vector_val<struct std::_Simple_types<unsigned char>>::_Vector_val<struct std::_Simple_types<unsigned char>>(void)")]
	public unsafe static void* Vector_val_struct_std_Simple_types_unsigned_char_Constructor(void* @this)
	{
		return AssetRipper.Conversions.FastPng.GlobalFunctions.Vector_val_struct_std_Simple_types_unsigned_char_Constructor.Invoke(@this);
	}

	[MangledName("?_Tidy@?$vector@EV?$allocator@E@std@@@std@@AEAAXXZ")]
	[DemangledName("private: void __cdecl std::vector<unsigned char, class std::allocator<unsigned char>>::_Tidy(void)")]
	[CleanName("Tidy")]
	public unsafe static void Tidy_iono57d(void* @this)
	{
		AssetRipper.Conversions.FastPng.GlobalFunctions.Tidy_iono57d.Invoke(@this);
	}

	[MangledName("?fpng_pixel_zlib_raw_decompress@fpng@@YA_NPEBEIIPEAEIIII@Z")]
	[DemangledName("bool __cdecl fpng::fpng_pixel_zlib_raw_decompress(unsigned char const *, unsigned int, unsigned int, unsigned char *, unsigned int, unsigned int, unsigned int, unsigned int)")]
	public unsafe static bool fpng_pixel_zlib_raw_decompress(void* pSrc, int src_len, int zlib_len, void* pDst, int w, int h, int src_chans, int dst_chans)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.fpng_pixel_zlib_raw_decompress.Invoke(pSrc, src_len, zlib_len, pDst, w, h, src_chans, dst_chans);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?prepare_dynamic_block@fpng@@YA_NPEBEIAEAI1AEA_KPEAII@Z")]
	[DemangledName("bool __cdecl fpng::prepare_dynamic_block(unsigned char const *, unsigned int, unsigned int &, unsigned int &, unsigned __int64 &, unsigned int *, unsigned int)")]
	public unsafe static bool prepare_dynamic_block(void* pSrc, int src_len, void* src_ofs, void* bit_buf_size, void* bit_buf, void* pLit_table, int num_chans)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.prepare_dynamic_block.Invoke(pSrc, src_len, src_ofs, bit_buf_size, bit_buf, pLit_table, num_chans);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}

	[MangledName("?build_decoder_table@fpng@@YA_NIPEAEPEAI@Z")]
	[DemangledName("bool __cdecl fpng::build_decoder_table(unsigned int, unsigned char *, unsigned int *)")]
	public unsafe static bool build_decoder_table(int num_syms, void* pCode_sizes, void* pTable)
	{
		bool result;
		try
		{
			result = AssetRipper.Conversions.FastPng.GlobalFunctions.build_decoder_table.Invoke(num_syms, pCode_sizes, pTable);
		}
		catch
		{
			StackFrameList.ExitToUserCode();
			throw;
		}
		StackFrameList.ExitToUserCode();
		return result;
	}
}
